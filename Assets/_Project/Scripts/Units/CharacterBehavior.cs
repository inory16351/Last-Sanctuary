...
namespace LastSanctuary.Units
{
    ...
    public class CharacterBehavior : MonoBehaviour
    {
        ...
        /// "사거리 밖에 서서 영영 못 때리는" 상태는 생기지 않는다.
        ///
        /// ★★★ <b>근거리 유형은 «떨어져 서기» 를 아예 쓰지 않는다</b> (2026-08-26 · 유저 리포트:
        /// *"중위나 후방에서 근접 공격 유형 설정할경우 사거리가 늘어나는거 같은데"*).
        ///
        /// <b>왜 늘어난 것처럼 보였나</b> — 이 함수의 옛 주석은 «근거리는 최대 사거리가 곧
        /// 접촉 거리라 후방이어도 결국 붙는다» 고 적어 두었는데 <b>그 전제가 틀렸다</b>:
        /// <code>
        ///   근거리 사거리(attackRange)      1.2 타일   ← 중심에서 중심까지
        ///   실제 접촉 거리                  0.5~0.9    ← 콜라이더가 닿는 거리
        ///   때릴 수 있는 거리(InAttackRange) 1.2 + 상대 반지름
        /// </code>
        /// 그래서 후방·중위의 근거리 캐릭터는 <b>1.2 타일까지 뒷걸음질쳐</b> 자리를 잡고
        /// (<see cref="UnitCombat.ChaseDestination"/> 는 «타겟에서 want 만큼 떨어진 점» 이라
        /// 이미 붙어 있으면 <b>물러난다</b>), 거기서 «상대 반지름» 덕에 여전히 때린다 —
        /// 화면에는 <b>칼이 안 닿는 거리에서 휘두르는</b> 것으로 보인다. 그것이 «사거리가
        /// 늘어난 것 같다» 의 정체다.
        ///
        /// ★ 포지션의 뜻은 <b>«전열의 어디에 서는가»</b> 이고, 그것은 이미
        ///   <see cref="GetPositionBand"/>·<see cref="TryPickSupportSpot"/> 가 «구역 안의 자리» 로
        ///   지킨다. «교전 중 거리 두기» 는 <b>원거리·마법·치유</b>처럼 떨어질 이유가 있는
        ///   유형에만 뜻이 있다.
        /// ⚠ 그래서 근거리는 <b>포지션과 무관하게 0</b>이다 — 전열은 서는 자리로, 교전은
        ///   붙어서. 후방 근거리가 «앞으로 나가 싸운다» 는 뜻이 아니다(적이 구역에 들어올
        ///   때까지는 여전히 뒤쪽 구간에 서 있는다).
        /// </summary>
        float StandoffFor(DamageableUnit foe)
        {
            float range = _combat.EffectiveAttackRange;

            // ★ 근거리는 «거리 두기» 자체가 없다 — 위 ★★★ 참조.
            if (_combat.AttackType == TacticalAttackType.Melee) return 0f;
