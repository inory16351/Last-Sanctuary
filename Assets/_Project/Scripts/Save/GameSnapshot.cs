...
namespace LastSanctuary.Save
{
    ...
    public class GameSnapshot : MonoBehaviour
    {
        ...
        {
            // ★★ 2026-08-27 — <b>엔딩 명단도 «이어하기» 에서 오염됐다</b>.
            //   <see cref="RunRecord"/> 는 판 전역 static 이고 비우는 곳이
            //   <see cref="RunResetService.ClearRunState"/> 하나인데, <b>이어하기는 그 문을
            //   지나지 않는다</b>. 그래서 「판 A 에서 셋이 죽음 → 로비 → 이어하기」 를 하면
            //   판 B 의 엔딩 «떠나간 이들» 에 <b>판 A 의 전사자</b>가 그대로 실렸다 —
            //   그 클래스의 ⚠⚠ 가 경고한 바로 그 사고이고, 경로만 새로 하나 있었던 것이다.
            //   ⚠ 전사자는 세이브에 담기지 않으므로 <b>되살릴 수는 없다</b>(비우는 것이
            //     최선이다) — 이어한 판의 엔딩은 «이어한 뒤에 죽은 이들» 만 싣는다.
            RunRecord.Clear();
        ...
        void RestoreCharacters(SaveData data)
        {
            ...
            // ══════════════════════════════════════════════════════════════
            //  ★★★ <b>«이어하기» 도 판의 시작이다</b> (2026-08-27 · 유저 리포트:
            //  *"첫번째 생성 때에도 고유한 자신의 이름이 아니라 랜덤 생성 이름을 가지고 나온다"*)
            // ══════════════════════════════════════════════════════════════
            //  «두 번째 등장은 다른 이름»(<see cref="Units.CharacterAltNames"/>)의 등장 기록은
            //  <b>판 전역 static</b> 이고, 비우는 자리는 <see cref="RunResetService.ClearRunState"/>
            //  하나다. 그런데 <b>이어하기는 그 문을 지나지 않는다</b> — 그래서 기록이
            //  <b>두 군데서</b> 오염돼 있었다:
            //
            //   ① <b>지난 판의 것이 그대로 남는다.</b> 로비로 나갔다가 이어하기를 누르면
            //      (같은 프로세스라 static 이 살아 있다) 인물 대부분이 이미 «1번 등장» 으로
            //      세어져 있어, 새로 만든 <b>첫 캐릭터가 곧바로 대체 이름</b>을 달고 나온다.
            //   ② <b>바로 위에서 지운 시작 캐릭터 3명</b>도 «등장» 으로 세어졌다. 세이브에 없는
            //      인물이면 그 셋은 <b>한 번도 존재한 적 없이</b> 1번 등장으로 남는다.
            //
            //  ★ 그래서 <b>여기서 비우고</b>, 아래 루프의 <c>MarkRestored</c> 가 세이브에 든
            //    사실(«이 인물은 이미 나왔다» · «이 이름은 이미 쓰였다»)로 <b>다시 채운다</b>.
            //    복원이 곧 «판 상태의 정본» 이므로, 비우고 다시 채우는 편이 지워야 할 것을
            //    하나씩 골라내는 것보다 틀릴 여지가 없다.
            //  ⚠ <b>위의 «캐릭터 0명» 조기 반환보다 뒤에 있어야 한다</b> — 그 경로는 시작
            //    캐릭터 3명을 <b>그대로 두고</b> 판을 잇는다. 거기서 비우면 그 셋이 «아직 안
            //    나온 인물» 이 되어, 죽고 다시 뽑혔을 때 <b>같은 이름으로 두 번</b> 태어난다.
