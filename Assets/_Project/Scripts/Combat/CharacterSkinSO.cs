...
namespace LastSanctuary.Combat
{
    ...
    public class CharacterSkinSO : ScriptableObject
    {
        ...
        // ------------------------------------------------------------------
        // ★★ 보호막 겹쳐 그리기 (2026-08-20 — 카이론 「타락한 육체」 80025)
        //
        // 유저 지시: *"카이론 실드 상시 표시(실드 효과 받고 있을때) … <b>분리된 이미지</b>
        // 사용해서 쉴드 있는 동안 <b>상시</b> 적용 되게 하고 <b>발동 이펙트로만</b> 스킬
        // 이펙트 쓰고"*.
        //
        // <b>왜 새 칸이 필요한가</b> — 지금까지의 이펙트 칸은 모두 «한 번 터지고 사라지는»
        // 것이다(`skill1Fx` · `impactFrames` · `healFxFrames`). 보호막은 성질이 다르다:
        // <b>상태가 남아 있는 동안 계속</b> 몸 위에 있어야 한다. 그것을 기존 칸으로 하면
        // «발동 순간에 한 번 뜨고 사라지는» 것이 되어 지시와 반대가 된다.
        //
        // ★ 원화도 <b>따로</b> 와 있다 — 카이론 시트의 분리 이펙트 구획에 황금 구체 6장이
        //   있다(`chiron_skin_build.py` 의 ★★ 「Shield」). 스킬 모션 안의 구체는 몸통과
        //   한 그림이라 겹쳐 그릴 수 없다. 유저가 «분리된 이미지» 라고 한 것이 이 이야기다.
        // ------------------------------------------------------------------

        [Header("보호막 (상시 겹쳐 그리기 — 카이론 「타락한 육체」)")]
        [Tooltip("★ <b>보호막이 남아 있는 동안 계속</b> 몸 위에 겹쳐 돌리는 그림.\n" +
                 "비어 있으면 아무것도 그리지 않는다 — 보호막을 갖는 다른 캐릭터가 생겨도 " +
                 "원화가 없으면 조용히 넘어간다(부활·소환과 같은 규칙).\n" +
                 "⚠ 이것은 «발동 이펙트» 가 아니다 — 그쪽은 skill1Fx 다")]
        public Sprite[] shieldFrames;

        /// <summary>보호막 겹쳐 그리기 프레임. 없으면 null.</summary>
        public Sprite[] Shield() => HasFrames(shieldFrames) ? shieldFrames : null;

        /// <summary>이 외형이 보호막 그림을 갖고 있는가 (UI·연출이 먼저 묻는다).</summary>
        public bool HasShieldArt => HasFrames(shieldFrames);
        ...
        // ------------------------------------------------------------------
        // ★★ 몸집 <b>보정</b> — 캐릭터끼리 «사람 키» 를 맞추는 칸 (2026-08-21)
        //
        // <b>왜 필요한가</b> — 캐릭터는 전원 <c>renderHeightTiles</c> 2.15 로 그려진다. 즉
        // <b>상자(알파 경계)</b> 높이는 이미 전원 같다. 그런데 상자 안에 든 것이 캐릭터마다
        // 다르다 — 아루는 머리 위 <b>후광</b>, 불칸은 <b>지팡이</b>가 상자를 위로 늘린다.
        // 그래서 상자가 같아도 <b>사람 키</b>는 다르게 나온다(실측: 불칸이 프레이야보다 9% 작다).
        //
        // ★ <b>왜 <see cref="contentSizeTiles"/> 를 안 고치나</b> — 그 칸은 «측정값이라 항상
        //   덮어쓴다»(<c>Tools/measure_skin_tiles.py</c>). 거기에 사람의 판단을 적으면
        //   다음에 그 스크립트를 돌릴 때 <b>조용히 원복된다</b>. 그래서 이 프로젝트가 이미
        //   갖고 있는 구분(«실측값» vs «표시 크기 — 사람이 정하는 값»)에 맞춰
        //   <b>표시 쪽에</b> 칸을 하나 뒀다. 측정 스크립트는 이 칸을 건드리지 않는다.
        //
        // ⚠ <b>판정도 같이 움직인다</b> — <see cref="LastSanctuary.Combat.CharacterAnimator.ColliderSizeTiles"/>
        //   가 «그려진 크기» 를 돌려주므로, 이 값을 키우면 근접 거리·선택 판정도 따라 커진다.
        //   그것이 이 프로젝트의 규칙이다(«보이는 몸집 = 판정 몸집»).
        // ------------------------------------------------------------------

        [Header("몸집 보정 (사람이 정하는 값 — 측정 스크립트가 안 건드린다)")]
        [Tooltip("이 외형에만 곱하는 <b>추가 배율</b>. 1 = 보정 없음.\n" +
                 "후광·지팡이처럼 «사람이 아닌 것» 이 상자를 늘려서 캐릭터가 남들보다 작아 " +
                 "보일 때 1 보다 크게 준다.\n" +
                 "기준은 프레이야(유저 확정 2026-08-21) — 실측 사람 키 1.92타일")]
        [Range(0.5f, 2f)] public float bodyScaleAdjust = 1f;

        /// <summary>
        /// <see cref="bodyScaleAdjust"/> 를 안전하게 읽는다 — 옛 에셋은 이 칸이 <b>0</b>
        /// (직렬화 기본값)일 수 있고, 0 을 곱하면 <b>유닛이 사라진다</b>.
        /// </summary>
        public float BodyScaleAdjust => bodyScaleAdjust > 0.0001f ? bodyScaleAdjust : 1f;
