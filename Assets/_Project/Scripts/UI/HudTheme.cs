...
namespace LastSanctuary.UI
{
    ...
    public static class HudTheme
    {
        ...
        ///
        /// ══════════════════════════════════════════════════════════════════
        ///  ⚠⚠ <b>2026-08-27 — <c>field = T(key, field)</c> 는 «키가 표에 있을 때만» 안전하다</b>
        /// ══════════════════════════════════════════════════════════════════
        /// 이날 창 열넷이 <c>OnLanguageChanged</c> 를 구독하면서 <c>LocalizeLabels()</c> 가
        /// <b>판마다 여러 번</b> 불리게 됐다(예전에는 <c>Awake</c> 에서 한 번뿐이었다).
        /// 그 관용구는 이렇게 생겼다:
        /// <code>
        ///   titleText = HudTheme.T("ui_x_title", titleText);   // 폴백 = 지금 값
        /// </code>
        /// <b>키가 표에 있으면</b> 언제나 표의 값이 돌아오므로 몇 번을 불러도 같다 —
        /// 한국어 ↔ 영어를 오가도 정상이다.
        /// <b>그런데 키가 표에 없으면</b> 첫 호출에서 폴백(= 인스펙터의 한국어)이 그대로 남고,
        /// 그 값이 <b>다음 호출의 폴백</b>이 된다. 그 상태에서 누군가 키를 <b>영어만</b> 채우면
        /// «영어로 갔다가 한국어로 못 돌아오는» 칸이 생긴다(182-3절이 <c>HelpCardPanel</c> 에서
        /// 겪은 것과 같은 종류다 — 그 창은 <c>_defaults</c> 에 원문을 붙잡아 두는 것으로 풀었다).
        ///
        /// ★ <b>그래서 규약은 «키를 먼저 표에 넣는다»</b> 이다. 코드가 부르는 키가 전부 표에
        ///   있는지는 <c>Tools</c> 의 검산이 세어 준다(2026-08-27 실측 — 빠진 키 0개).
        ///   여러 번 불려도 확실히 안전해야 하는 창은 <c>HelpCardPanel</c> 처럼
        ///   <c>_defaults</c> 를 두면 된다.
        /// </summary>
