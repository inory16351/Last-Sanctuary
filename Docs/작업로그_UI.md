...
## UI-63. ★★★ 발굴 표식이 <b>보이지 않던 두 번째·세 번째 이유</b> · 미니맵 느낌표 · 유물 18종 추가 (2026-08-25, 21차)
...
> *"유물 드랍이 마르지 않도록 조금 더 추가해줘"*
> *"발굴 가능 칸이 발견되면 느낌표를 미니맵에도 추가해줘"*

160절이 «칸이 <b>한 개도 안 놓이던</b>» 것을 고쳤는데, 그것만으로는 <b>여전히 화면에
아무것도 안 뜬다</b>. 칸이 놓이는 것과 <b>사람이 그것을 찾는 것</b>은 다른 문제였고,
거기에 <b>둘</b>이 더 걸려 있었다.

**고친 C#** — `Relics/RelicDigService.cs` · `UI/MinimapPanel.cs`
**씬 변경** — `HUD_Minimap/DigMarks` + `MarkTemplate` **MCP 로 신설** ·
`RelicDigService` 칸 셋 조정 · `save_scene`
**표 변경** — 유물 <b>48 → 66종</b> (볼트 유물 표 · 에셋 · 아이콘 · 스트링 키)

---

### UI-63-1. ★★★ 둘째 이유 — <b>「지금 보이는가」로는 사실상 아무도 못 찾는다</b>

```csharp
if (!_fog.IsVisible(s.Cell)) continue;      // ← 지금 누군가의 시야 «칸» 인가
```

발굴 칸은 서로 10타일 이상 떨어져 흩어져 있고, 탐험은 안개를 걷으며 <b>지나가 버리는</b>
것이다. 그 <b>순간</b>에 시야 칸이 정확히 겹치지 않으면 끝이다 — 안개는 이미 걷혔으니
그 자리는 <b>다시는 «지금 보이는» 상태가 되지 않는다</b>.

★ <b>이 프로젝트는 같은 결론을 이미 한 번 냈다.</b> `EpicSubjugationService` 가
  «폴리르만 안 뜬다» 를 쫓다가 `IsVisibleWorld` → `IsExploredWorld` 로 바꾼 그 판단이다
  (그 클래스의 ★★★). 이 게임의 안개는 «한 번 밝히면 기억되는» 방식이고, «찾았다» 도
  같은 성질이어야 앞뒤가 맞는다.

→ `IsExplored`(가 본 자리)로 바꿨다. 이제 <b>맵을 넓혀 갈수록 표식이 하나씩 는다</b>.

### UI-63-2. ★★★ 셋째 이유 — <b>맵 전체에 흩뿌리면 대부분이 «평생 못 갈 곳» 이다</b>

자리 고르기는 «맵 안 아무 데나» 였다. 그런데 맵은 <b>320×320(102,400칸)</b> 이고
플레이어가 실제로 오가는 곳은 <b>성역 주변 100타일 남짓</b>이다(에픽 서식지가 100~160,
집결지·방어는 훨씬 안쪽). 24칸을 맵 전체에 뿌리면 <b>대다수가 아무도 안 가는 구석</b>에 놓인다.

실제로 그랬다 — 160절의 고침 뒤 플레이에서 «칸 24개 배치» 는 찍혔는데
<b>`DigOverlay` 아래 복제가 한 개도 안 생겼다</b>(실측). 놓이기는 했지만 <b>닿지 않는 곳</b>이었다.

→ <b>바깥 한계</b>를 만들었다: `maxDistanceFromNexus`(0 = 맵 전체 · 기본 110타일).
  더불어 씬 값을 <b>24 → 40칸</b>으로 올렸다. 반지름 110의 고리 안에 40칸이면
  대략 <b>950칸에 하나</b>라 탐험하다 보면 몇 개는 눈에 들어온다.

⚠ <b>가운데 한계(14타일)는 그대로다</b> — 성역 코앞에서 파는 것은 «탐험의 보상» 이 아니다.

### UI-63-3. 느낌표와 바운스는 <b>이미 있었다</b>

147절이 만든 것이 전부 살아 있다 — 주황 느낌표 원화(`Resources/DigMarker/dig_marker`) ·
«바닥에서 쉬는 시간» 이 있는 포물선 바운스 · 파는 중에는 안 튀는 규칙까지.
<b>보일 기회가 없었을 뿐이다.</b> 그래서 이번에 코드는 손대지 않고 <b>보이게만</b> 했다.

### UI-63-4. ★★ 미니맵 느낌표 — <b>픽셀이 아니라 겹치는 한 겹</b>

미니맵은 «맵 한 칸 = 텍스처 한 픽셀» 이라 느낌표를 픽셀로 그리면 <b>3×5짜리 뭉개진 점</b>이
된다(시야 사각형은 굵기 1~2픽셀이라 픽셀로 그려도 되지만 <b>글자 모양은 다르다</b>).
그래서 텍스처 위에 <b>UI 이미지 한 겹</b>을 얹었다.

```
HUD_Minimap
├─ Title · View(RawImage) · Border · ViewBorder
└─ DigMarks        ← 신설 · View 와 «같은 자리·같은 크기» (좌표가 맞으려면 반드시)
    └─ MarkTemplate ★모체(비활성) — 복제는 스크립트가 한다
```

★ 좌표는 <b>정규화</b>로 옮긴다 — `TryScreenToWorld`(미니맵 클릭 이동)가 쓰는 그 비율의
  <b>역</b>이다. 텍스처 크기·화면 크기·줌과 무관하게 맞는다.
★ 칸 <b>가운데</b>를 가리킨다(+0.5) — 안 하면 한 칸씩 왼쪽·아래로 치우친다.
★ <b>튀는 리듬을 복사하지 않았다</b> — `RelicDigService.BounceFor(index, scale)` 를 불러 쓴다.
  계산이 두 벌이 되면 인스펙터에서 주기를 바꿨을 때 <b>한쪽만 따라간다</b>.
  미니맵은 좁으므로 <b>0.35배</b>로만 튄다(인스펙터 `digMarkBounceScale`).
★ <b>원화를 코드가 꽂는다</b> — 스프라이트 «참조» 는 MCP 로 인스펙터에 넣을 수 없다(8절 4번).
  `RelicDigService.MarkerSprite` 를 그대로 받아 쓴다 — 화면 표식과 <b>같은 그림</b>이 보장된다.
★ 이미 파러 간 칸은 <b>회색</b>이다(화면 표식과 같은 규칙 — 주의를 끌 일이 끝났다).
⚠ 복제는 <b>재사용</b>한다. 남는 것은 꺼 둔다 — 발견 칸이 늘었다 줄었다 할 때마다
  만들고 지우면 GC 가 튄다.

### UI-63-5. ★★ 유물 <b>48 → 66종</b> — 중복을 금지했으면 <b>풀을 넓혀야 한다</b>

160절이 중복 획득을 막았다. 그 대가가 이것이다 — <b>다 모으면 그 등급이 통째로 마른다</b>.

| 등급 | 전 | 후 | 마르면 무엇이 죽는가 |
|---|---:|---:|---|
| 일반 (발굴·처치) | 16 | **24** | 발굴 결과의 <b>16%</b> · 처치 드랍 1.2% |
| 레어 (발굴·처치) | 13 | **20** | 발굴 결과의 <b>4%</b> · 처치 드랍 0.2% |
| 에픽 <b>발굴 전용</b> | 6 | **9** | 발굴 결과의 1% — <b>가장 먼저 마르는 자리</b> |
| 에픽 보스 고유 | 10 | 10 | (보스마다 하나라 늘릴 것이 아니다) |
| 에픽 사건 전용 | 3 | 3 | (사건 보상 전용) |

★ <b>값의 대역을 지켰다</b> — 일반은 «한 칸이면 1~4 · 두 칸이면 각 1~2», 레어는
  «능력치 4 · 비율 8~20». <b>풀을 넓히는 것이지 세게 만드는 것이 아니다.</b>
★ <b>은유의 축도 지켰다</b>(표 Info 시트) — 일반 = 비특이 면역(마른 기침·튼 살·식은 땀),
  레어 = 특이 면역(보체의 그물·굳어진 항원·증식 신호),
  발굴 전용 에픽 = <b>누구의 것도 아닌, 그냥 묻혀 있던 것</b>(잊힌 항체·묻힌 심장·굳어버린 숨).
★ <b>새 효과 타입을 만들지 않았다</b> — 27종이 이미 있고 조합만으로 18종이 나온다.
  타입을 늘리면 `RelicEffectService` 에 가지를 더해야 하고, 빠뜨리면 «장착은 되는데
  아무 일도 안 일어나는» 유물이 된다(그 등록기가 경고하는 그것).

⚠ <b>아이콘 키가 겹쳐 검산이 한 번 막았다</b> — `relic_nail`·`relic_complement` 가
  기존 것과 같았다. 생성기가 «두 유물이 같은 그림을 씁니다» 로 <b>표를 쓰지 않고 멈췄다</b>.
  `relic_hardnail`·`relic_complementnet` 로 고쳤다. <b>검산이 일을 했다.</b>

### UI-63-6. 표 → 에셋 → 스트링, 세 단계를 다 돌렸다

```
Tools/gen_relic_table.py    → 볼트 유물 표 (66행)
Tools/gen_relic_assets.py   → Resources/Relics 66개 + 임시 아이콘 66장
Tools/gen_string_table.py   → 스트링 키 표 + StringTable.txt
```

⚠ 표를 통째로 다시 쓰는 스크립트라 <b>159절에 손으로 고친 「삼킨 티끌」이 되돌아가지
  않는지</b> 먼저 확인했다 — 표를 다시 쓰기 <b>전</b> 사본을 떠서 <b>행 단위로 비교</b>했다:
  기존 48행 중 <b>값이 바뀐 것 0건</b> · 사라진 것 0건 · 새 id 18개.
  (159절에서 생성기에도 같은 값을 넣어 둔 것이 여기서 값을 했다.)

### UI-63-7. ⚠⚠ <b>표를 다시 굽다가 두 가지를 밟았다</b> — 되돌린 기록

생성기를 돌리는 것이 «표만 다시 쓰는» 일이 아니었다. 둘 다 <b>사람 손이 들어간 결과물</b>을
덮어썼고, 둘 다 <b>대조해서 잡았다</b>.

**① 유물 아이콘 48장이 원화였다.** 생성기의 «임시 아이콘» 은 <b>덮으라고</b> 만든 것이고
(그 파일 머리: *"원화가 오면 같은 파일 이름으로 덮으면 된다"*), 실제로 덮여 있었다.
다시 돌리니 <b>48장이 전부 임시 그림으로 되돌아갔다</b> — 픽셀 대조로 «최대 채널차 255 ·
16,384/16,384 픽셀이 다름» 이 나와 곧바로 드러났다.
→ 기존 48장은 <c>git restore</c> 로 <b>되살렸고</b>, <b>새 18장만</b> 남겼다.
★ guid 는 무사하다 — 생성기가 기존 <c>.meta</c> 의 guid 를 <b>읽어서 다시 쓴다</b>. 실측으로
  확인했다(`relic_scab` 의 guid 가 에셋·메타·HEAD 셋 다 같다).

**② 156절의 「넥서스 → 성역」 개명이 한 줄 되돌아갔다.** 생성기 안의 발굴 대사 한 줄이
아직 «넥서스» 였다. 156절이 <b>에셋만</b> 고치고 <b>생성기는 안 고쳤기</b> 때문이다 —
159-9절이 적어 둔 «표를 안 고치면 되돌아간다» 의 <b>정확히 그 사례</b>가 한 번 더 났다.
→ 생성기의 그 줄을 고치고 다시 구웠다. 은유 설명 두 줄도 같이 정리했다.

⚠ <b>교훈</b> — «생성기를 돌린다» 는 <b>되돌릴 수 있는 일이 아니다</b>. 이번에는 돌리기 전에
  표 사본을 떠 뒀고(161-6) 결과를 <b>행 단위·픽셀 단위로 대조</b>했기 때문에 둘 다 잡혔다.
  대조를 안 했으면 «유물 그림이 왜 촌스러워졌지» 를 한참 뒤에 알았을 것이다.

### 검증

* `recompile_scripts` **에러 0 · 경고 0** · 플레이 중 콘솔 **에러 0 · 경고 0**
  (경고 0 은 «`DigMarks`·`MarkTemplate` 을 찾았다» 는 뜻이기도 하다 — 못 찾으면 경고한다)
* 유물 생성기 검산 통과 — 66종 · 일반 24 · 레어 20 · 에픽 22 · 효과 타입 27 · 발굴 결과 가중치 합 100
* 표 대조 — 기존 48행 <b>무손상</b>, 「삼킨 티끌」 여전히 `value_01 5 · value_02 40`
* 스트링 — 새 유물 이름·설명·서사가 `StringTable.txt` 에 들어갔고,
  159절의 `character_title_9011`(대마법사) · `skill_name_2003~2009` 도 <b>그대로 살아 있다</b>
* 씬 실측 — `HUD_Minimap/DigMarks`(View 와 같은 자리) · `MarkTemplate`(비활성 · 14x14 · 주황)

### 아직 확인 못 한 것 (유저가 볼 것)

1. ★★ <b>표식이 실제로 뜨는 데는 시간이 걸린다.</b> 발굴 칸은 성역에서 <b>14~110타일</b>
   고리에 있고 «가 본 자리» 라야 뜬다 — 부대를 <b>탐험</b>으로 돌려 두고 몇 분 지나야
   하나씩 나타난다. 너무 뜸하면 `GameSystems ▸ RelicDigService` 의
   `digSiteCount`(40) 를 올리거나 `maxDistanceFromNexus`(110) 를 줄이면 된다.
2. ★ <b>미니맵 느낌표의 크기는 눈으로 판단해야 한다.</b> 14px 로 잡았는데 미니맵이
   280×280 남짓이라 조밀하면 겹칠 수 있다. `HUD_Minimap ▸ MinimapPanel ▸ digMarkPixels` 로 조절.
3. ★★ <b>새 유물 18종의 아이콘만 «임시 그림»</b> 이다(생성기가 절차적으로 그린 네모).
   기존 48장은 원화이고 이번에 <b>되살려 두었다</b>(161-7). 원화가 오면 같은 파일 이름으로
   덮으면 된다 — 표의 `icon` 칸이 그 이름이다:
   `relic_cough` `relic_vein` `relic_stretchmark` `relic_coldsweat` `relic_hardnail`
   `relic_yawn` `relic_swolleneye` `relic_chappedlip` · `relic_hungrycell`
   `relic_complementnet` `relic_autophagy` `relic_metabolism` `relic_antigen`
   `relic_mitogen` `relic_cornea` · `relic_forgottenab` `relic_buriedheart` `relic_heldbreath`
   ⚠ <b>생성기를 다시 돌리면 원화가 또 덮인다</b>(161-7 ①). 돌린 뒤에는 반드시
     `git status` 로 <b>기존 아이콘이 M 으로 뜨는지</b> 보고, 뜨면 그 48장은 되돌릴 것.
4. **새 유물 18종의 값은 첫 값**이다. 대역은 맞췄지만 실제로 끼워 보고 판단해야 한다.
   `Tools/gen_relic_table.py` 에서 고치고 세 스크립트를 다시 돌리면 된다.

### 씬반영요청 목록

- 없음 (MCP 로 만들고 `save_scene` 했다)

---

## UI-64. 발굴 자리를 <b>맵 전체</b>로 · 칸 수 40 → 110 · ★★ 「걸어갈 수 있는 칸」만 (2026-08-25, 22차)

### 무엇을 / 왜

> 유저 지시: *"발굴 가능 칸 뜨는 범위 중앙 건물로부터 14타일부터 맵 끝까지(직사각형 범위
> 320까지)로 확장하고 맵내에 뜰 수 있는 발굴 가능 칸 확장된 구역 고려해서 재설정"*

161절이 «맵 전체에 흩뿌리면 대부분이 평생 못 갈 곳» 이라며 바깥 한계를 <b>110타일</b>로
좁혔는데, 유저가 <b>반대 방향</b>을 확정했다 — 맵 끝까지 쓰되 <b>칸 수를 같이 올린다</b>.
그 편이 «탐험할수록 새 표식이 나온다» 는 성질을 끝까지 살린다.

**고친 C#** — `Relics/RelicDigService.cs`
**씬 변경** — `RelicDigService` 칸 넷 (MCP) · `save_scene`
**표 변경** — 유물 표 `Info` 시트에 <b>「발굴 칸 «자리 고르기»」·「발굴 칸을 «찾는다» 는 것」</b> 두 절 신설

---

### UI-64-1. 범위 — 바깥 한계를 <b>0(맵 전체)</b> 으로

`maxDistanceFromNexus` 의 뜻은 그대로 두고 <b>기본값만</b> 110 → <b>0</b> 으로 바꿨다.
0 은 «무제한» 이고, 자리 고르기가 애초에 맵 안의 칸만 뽑으므로 그것이 곧
<b>직사각형 320×320 끝까지</b> 다 — 유저가 말한 «직사각형 범위» 와 정확히 같다.

⚠ <b>안쪽 한계 14타일은 그대로</b>다(유저가 «14타일부터» 라고 못박았다).
  성역 코앞에서 파는 것은 «탐험의 보상» 이 아니다.

### UI-64-2. ★★ 칸 수 — <b>넓힌 만큼 올려야 한다</b>

이것이 «재설정» 의 알맹이다. <b>넓이만 늘리고 칸 수를 그대로 두면 밀도가 묽어져
성역 근처에서 하나도 안 보인다</b> — «넓혔더니 오히려 못 찾는» 것이 된다.

| | 전 (161절) | 후 |
|---|---|---|
| 범위 | 성역 반경 14~**110** | 성역 14~**맵 끝** (직사각 320×320) |
| 넓이 | 원 π·110² ≈ **38,013**칸 | 사각 **102,400**칸 (**2.69배**) |
| 칸 수 | 40 | **110** (2.75배) |
| 밀도 | 1칸 / 약 950타일 | 1칸 / 약 930타일 — <b>거의 같다</b> |

★ <b>밀도를 유지하는 쪽으로 맞췄다.</b> 성역 근처에서 표식이 나오는 빈도가 예전과
  같아야 «넓혔다» 가 손해로 느껴지지 않는다. 넓힌 만큼은 <b>순수한 추가분</b>이다.
⚠ 간격(`minSpacing` 10타일)은 그대로다. 110칸 × 반경 5 배타 넓이 = 약 8,600칸이라
  102,400 안에서 넉넉하다 — 실측으로 <b>169번 시도만에 110칸</b>을 채웠다.
⚠ `placementAttempts` 는 4,000 → <b>12,000</b> 으로 올렸다. 실측은 169번이지만
  지형이 험한 씨앗에서 꼬리가 길어질 수 있고, 한 판에 한 번 도는 순환이라 비용이 없다.
⚠ <b>칸은 파면 사라진다</b>. 다시 생기지 않으므로 110 이 곧 <b>한 판의 발굴 횟수 상한</b>이다.

### UI-64-3. ★★★ 넓히면서 생긴 새 구멍 — <b>「못 가는 주머니」</b>

<c>IsCellPlaceable</c> 는 «<b>벽이 아닌가</b>» 만 본다. «<b>거기까지 갈 수 있는가</b>» 는 안 본다.
반경 110 안쪽은 성역 주변이라 대체로 뚫려 있었지만, <b>맵 끝까지 넓히면</b> 벽에 둘러싸인
주머니가 사정권에 들어온다. 거기 놓인 표식은 <b>지시해도 아무도 도착하지 않아</b>
영영 남는다 — 유저에게는 «발굴이 또 고장났다» 로 보인다.

→ 조건에 <b>다섯째</b>를 더했다: `FlowFieldService.IsCellReachable(cell)`.
  이 프로젝트에 이미 «성역까지 갈 수 있는 칸» 지도가 있으므로 <b>새로 만들 것이 없다</b>.

### UI-64-4. ⚠⚠ 그런데 그 지도가 <b>아직 안 구워져 있었다</b>

실측(콘솔 시각)으로 <b>발굴 서비스가 플로우 필드보다 먼저</b> 돌고 있었다:

```
15:04:34.099  [유물] 발굴 가능 칸 24개 배치      ← 먼저
15:04:34.155  [FlowField] 320x320 생성           ← 나중
```

유니티는 «모든 <c>Awake</c> 가 모든 <c>Start</c> 보다 먼저» 만 보장하고 <b>오브젝트 사이의
<c>Start</c> 순서는 보장하지 않는다</b>. 그대로 두면 도달 검사가 <b>있으나 마나</b> 였다.

→ <b>자리 고르기를 «준비되면 그때» 로 미뤘다.</b> <c>Start</c> 는 예약만 하고,
  <c>Update</c> 가 매 프레임 플로우 필드를 확인해 준비된 순간에 고른다.
★ <b>실행 순서 설정(Script Execution Order)으로 못박지 않았다</b> — 그건 <b>보이지 않는
  의존</b>이라 다음 사람이 이 코드만 읽고는 알 수 없다. 코드가 스스로 확인하는 편이 낫다
  (`MapGenerator.ResolveStartupSeed` 가 «밀지 말고 당긴다» 로 내린 것과 같은 판단).
⚠ <b>영원히 기다리지 않는다</b> — `pickWaitSeconds`(기본 5초) 안에 준비가 안 되면
  <b>도달 검사 없이</b> 고르고 <b>경고를 남긴다</b>. 플로우 필드가 없는 씬에서 기능이
  통째로 죽는 것보다 낫다(160-2절의 «조용한 실패는 기능이 없는 것과 구별되지 않는다»).

### UI-64-5. 로그가 <b>무엇을 어떻게 골랐는지</b> 말한다

```
[유물] 발굴 가능 칸 110개 배치 (목표 110 · 시도 169 · 못 가는 자리 0건 버림 · 성역에서 14~맵 끝까지)
```

★ 한 줄에 <b>결과·노력·거른 것·범위</b>가 다 있다. 값이 이상할 때 <b>어느 조건이 물었는지</b>를
  이 줄만 보고 안다 — «못 가는 자리 N건 버림» 이 크면 지형이 막힌 씨앗이고, «시도» 가
  한계에 닿으면 간격이나 칸 수가 과하다.
★ «도달 검사 없음» 이라고 찍히면 <b>162-4 의 기다림이 실패한 것</b>이다.

### UI-64-6. 표에 규칙을 적었다 — <b>코드가 가리키던 절이 없었다</b>

`PickSites` 의 주석은 예전부터 *«표 Info 시트 «자리 고르기»»* 를 가리키고 있었는데
<b>그 절이 표에 없었다</b>. 규칙이 코드에만 있으면 밸런스를 만지는 사람이 못 찾는다.

→ 유물 표 `Info` 시트에 두 절을 신설했다:
  · <b>「발굴 칸 «자리 고르기»」</b> — 조건 다섯 · 지금 값 · «칸 수는 범위와 함께 움직여야 한다»
  · <b>「발굴 칸을 «찾는다» 는 것」</b> — 가 본 자리라야 뜬다 · 화면과 미니맵 두 곳

⚠ <b>수치 자체는 표에 넣지 않고 씬에 두었다</b>(값은 적어 두되 «정본은 인스펙터» 라고 명시).
  플레이하면서 만지는 값이라 인스펙터가 맞는 자리다 — 중립 사냥 성장이
  `NeutralGrowthService` 에 있는 것과 같은 이유이고, 표에도 두면 <b>어느 쪽이 이기는지</b>를
  매번 다시 물어야 한다.

### 검증

* `recompile_scripts` **에러 0 · 경고 0** · 플레이 중 콘솔 **에러 0**
* ★★★ 실측 — <b>`발굴 가능 칸 110개 배치 (목표 110 · 시도 169 · 못 가는 자리 0건 버림 ·
  성역에서 14~맵 끝까지)`</b>. 목표를 <b>다 채웠고</b>, 도달 검사가 <b>실제로 돌았다</b>
  («도달 검사 없음» 이 아니다 = 162-4 의 기다림이 성공했다).
* 로그가 찍히는 <b>시각</b>이 바뀐 것으로 미룬 것이 확인된다 — 예전에는 성역 생성보다
  <b>앞</b>이었는데 이제 <b>뒤</b>다.
* 유물 표 생성기 검산 통과 · <b>아이콘은 건드리지 않았다</b>(161-7 ①의 재발 방지 —
  `gen_relic_table.py` 는 xlsx 만 쓰고 `gen_relic_assets.py` 를 안 돌렸다. `git status` 로
  RelicIcons <b>0건</b> 확인)

### 아직 확인 못 한 것 (유저가 볼 것)

1. ★★ <b>표식 110개가 한꺼번에 보이면 시끄러울 수 있다.</b> 지금은 «가 본 자리» 만 뜨므로
   초반에는 몇 개뿐이지만, 맵을 다 밝히면 <b>화면과 미니맵에 최대 110개</b>가 동시에 뜬다.
   특히 미니맵(280×280 남짓)에 110개는 빽빽할 수 있다. 거슬리면 두 갈래로 조절한다 —
   `digSiteCount` 를 줄이거나, `MinimapPanel ▸ digMarkPixels`(14) 를 줄인다.
2. ★ <b>맵 끝의 칸은 «갈 수는 있지만 멀다».</b> 도달 검사는 통과해도 성역에서 160타일이면
   왕복이 길다. 부대를 그쪽으로 보내는 것이 손해로 느껴지면 `maxDistanceFromNexus` 를
   130~150 쯤으로 되돌리는 것이 중간값이다(0 = 맵 전체 · 110 = 161절 값).
3. **씨앗에 따라 «못 가는 자리» 가 나올 수 있다.** 이번 씨앗은 0건이었다. 로그의
   «못 가는 자리 N건 버림» 이 수백 건으로 크면 그 지형이 막힌 것이고, 그때도 110칸은
   채워지지만 시도 횟수가 는다(12,000 안에서).

### 씬반영요청 목록

- 없음 (MCP 로 값을 넣고 `save_scene` 했다)

---

## UI-66. 도움말에 <b>UI 그림</b>을 실제로 깔았다 · 분류 탭을 <b>밝은 것</b>으로 · 유물 아이콘 「닮은 짝」 여덟 교체 (2026-08-26, 23차)

> ⚠ 이 로그에 <b>UI-65 절이 없다</b> — 그 작업(엔딩·발굴 밸런스·단축키·아이콘 66종)은
> 커밋 메시지에만 남았다. 절 번호는 커밋 번호를 따라 <b>66</b> 으로 잇는다.
...
> ③ *"유사한 유물 아이콘 검토 후 겹치거나 유사하면 다른 이미지 아이콘 사용해서 변경(볼트에 있는 거 잘라서)"*
> ④ *"mcp 이용해서 수정 하고 테이블에 변경 값 반영"*

---

### UI-66-1. ★★ 도움말에는 <b>그림이 한 장도 안 깔려 있었다</b>

UI-61(픽셀 UI)이 «도움말에도 ui 이미지 사용해줘» 를 받아 <c>UiSkinApplier</c> 의 훑는
목록에 <c>Help_Root</c> 를 넣어 두었는데, <b>메뉴를 돌리지 않아 씬에 반영이 안 돼 있었다</b>.
실측 — <c>HUD_Help ▸ Image ▸ m_Sprite: null</c>, 색은 옛 단색 <c>0.07/0.08/0.10</c>.

그리고 <b>여백 쪽은 목록에조차 없었다</b>: <c>UiTextInset</c> 은 <c>UI_Root</c> 하나만
훑고 있었다. 그림을 깔면 창 테두리가 <b>23px</b>, 속판이 <b>10px</b> 생기는데 도움말 창의
글자는 가장자리 <b>12~18px</b> 에 있었으므로, 그냥 배선만 돌렸다면 <b>머리글·힌트·탭·목록·
상세가 전부 테두리 밑으로</b> 들어갔을 것이다.

→ 세 곳을 고쳤다.
* <c>UiSkinApplier.Roots</c> 를 <b><c>public</c></b> 으로 올리고 <c>UiTextInset</c> 이
  <b>그 목록을 그대로 쓰게</b> 했다. ★ 캔버스가 하나 더 늘어도 <b>한 곳만</b> 고치면 둘이 따라온다 —
  두 벌로 두었던 것이 이번 사고의 원인 그 자체다.
* <c>Tools/ui_text_clip_check.py</c>(정적 검사기)도 같은 두 캔버스를 보게 했다.
  덤으로 <b>경계를 <c>.png.meta</c> 에서 직접</b> 읽게 바꿨다 — 예전에는 <c>Temp/ui_sprite_cut.json</c>
  을 읽었는데 <b><c>Temp/</c> 는 유니티가 지우는 자리</b>라 이미 «파일 없음» 으로 죽어 있었다.
  하드코딩돼 있던 옛 프로젝트 경로도 <c>vault_path</c> 로 바꿨다.

### UI-66-2. 창을 <b>키워서</b> 여백을 만들었다 (좁히지 않았다)

<c>Tools/mcp_build_help_ui.py</c> 의 좌표를 전부 다시 잡았다. 상수 둘로 못박는다 —
<b><c>WIN_PAD = 27</c></b>(창 경계 23 + 숨통 4) · <b><c>BOX_PAD = 14</c></b>(속판 10 + 4).

| | 전 | 후 |
|---|---|---|
| 도움말 창 <c>HUD_Help</c> | 940×600 | <b>980×648</b> |
| 조언 카드 <c>HUD_HelpCard/Card</c> | 720×300 | <b>760×330</b> |
| 안내 말풍선 <c>HUD_HelpTour/Bubble</c> | 560×210 | <b>600×240</b> |

★ <b>안쪽을 좁히지 않고 바깥을 넓혔다.</b> 폭을 그대로 두고 여백만 주면 본문에 들어가는
  줄 수가 줄어 «안 가려졌는데 대신 넘친다» 가 된다. 넓힌 만큼이 정확히 테두리 몫이라
  <b>쓸 수 있는 넓이는 예전과 같다</b>.
⚠ 머리글을 16 내리면 <b>탭도 같이</b> 내려야 한다 — 안 내리면 겹친다(머리글 하단 64 ↔ 옛 탭 상단 56).
  그래서 탭·목록·상세가 줄줄이 8~12 씩 내려갔다.
⚠ 말풍선 크기를 씬에서 바꿔도 된다 — <c>HelpTourPanel.PlaceBubble</c> 은 <b>자리</b>만 잡고
  크기는 <c>sizeDelta</c> 를 <b>읽어서</b> 쓴다(확인함).
⚠⚠ <b>이 스크립트를 돌린 뒤에는 반드시 「배선」을 다시 돌린다</b> — <c>image()</c> 가 색을
  어두운 단색으로 되돌리기 때문에, 그림은 남아도 그 색이 곱해져 새까매진다. 파일 맨 위에 적었다.

### UI-66-3. ★★★ 분류 탭 — <b>색을 칠하면 그림이 새까매진다</b>

②의 «가시성이 너무 안 좋음» 은 두 겹이었다.

① 탭이 쓰던 <c>Btn_Panel_Normal</c> 의 속색은 <b><c>#212B38</c></b> 로 창 판때기와
  <b>한 단</b> 차이다 — 눌러야 할 것이 배경에 묻힌다.
② 더 나쁜 것: <c>HelpPanel.RebuildTabs</c> 가 <c>Background.color</c> 에 어두운
  <c>tabNormal</c>(0.13/0.17/0.22)을 <b>직접 칠하고</b> 있었다. 그림이 깔리면 그 색이
  그림에 <b>곱해져</b> 새까매진다 — <c>HudTheme.PaintButton</c> 이 이미 적어 둔
  <b>«그림을 넣었는데 안 보인다»</b> 의 정체 그대로다.

→ **밝은 계열 <c>Btn_Tab_*</c> 을 새로 만들었다** (<c>Tools/ui_make_tab_sprites.py</c> 신설).
  볼트의 버튼 시트는 여섯 벌뿐이고 <b>전부 이미 쓰고 있어서</b> 더 밝은 원화를 가져올 데가
  없다. 그래서 <b>있는 그림을 팔레트 안에서 한 단 올렸다</b> — 새 색을 만드는 것이 아니라
  <c>ui_sprite_cut.py</c> 의 16색 팔레트에서 <b>다음 칸</b>으로 옮기는 것이라 색 수가 안 는다.

```
  Btn_Tab_Normal ← Btn_Panel_Normal 한 단   속 #212B38 → #3D5468
  Btn_Tab_Hover  ← Btn_Panel_Hover  한 단   속 #2E4252 → #5A7186
  Btn_Tab_On     ← Btn_Panel_On     그대로  (청록 = 고른 것)
  Btn_Tab_Off    ← Btn_Panel_Off    그대로  (잠김은 어두운 것이 맞다)
```

⚠ <b>두 단은 안 된다</b> — 속이 <c>#94A3B3</c>(은빛)이 되어 흰 글자가 묻힌다(실측해서 되돌렸다).
⚠ <b>가장 어두운 <c>#0D0F14</c> 는 안 올린다</b> — 그것은 버튼의 <b>바깥 윤곽선</b>이라
  같이 밝히면 경계가 흐려져 «밝게 했더니 오히려 뭉개진다» 가 된다.
⚠ <b>평시와 올림을 둘 다</b> 올려야 한다. 평시만 올리면 <b>마우스를 얹는 순간 어두워지는</b>
  뒤집힘이 생긴다(1차 시도에서 실제로 그랬다).
★ <c>.meta</c> 는 <b>원본을 베껴</b> 쓴다 — 9-슬라이스 경계(22/0/22/0)·Point 필터·무압축이
  형제 버튼과 한 치도 달라선 안 된다. guid 만 경로에서 결정적으로 새로 만든다.

배선 쪽은 <b>한 줄</b>이면 된다 — <c>path.Contains("/Tabs/")</c> → <c>"Tab"</c>.
★ 이름을 하나하나 적지 않는 이유: 탭은 <c>TabTemplate</c> 을 복제해 런타임에 생기고
  (템플릿 예외), 복제본은 <b>스프라이트를 물려받으며</b>, <c>PaintButton</c> 은 계열을
  <b>붙어 있는 그림 이름에서</b> 읽는다. 그래서 이 한 줄이 «고른 탭/안 고른 탭» 까지 자동으로 맞춘다.

코드 쪽은 <c>Background.color = …</c> 를 <c>HudTheme.PaintButton(bg, On/Normal, 색)</c> 으로
바꿨다. 목록 행도 같은 함수를 지나가게 했다 — 행은 그림을 안 깔므로 <b>그 함수 안의
«그림이 없으면 색을 칠한다»</b> 로 떨어져 예전과 같이 보이고, 창마다 규칙이 갈리지 않는다.

### UI-66-4. 검증 — <b>눈이 아니라 자로</b>

* <c>LastSanctuary/UI/배선</c> — 창 12 · 판 42 · 버튼 82 · 막대 22 · 칸 4 · 액자 2.
  로그에 <b><c>버튼 Tab HUD_Help/Tabs/TabTemplate</c></b> 이 찍혔다.
* <c>LastSanctuary/UI/글자 여백</c> — <b>민 글자 2개, 둘 다 <c>UI_Root</c></b>.
  도움말 쪽은 <b>0개</b> = 66-2 의 좌표가 처음부터 테두리 안이었다는 뜻이다.
* <c>python Tools/ui_text_clip_check.py</c>(씬 YAML 정적 검사) — 삐져나옴 <b>7건, 전부
  <c>UI_Root</c></b>(대부분 가짜 양성 — 짧은 왼쪽 정렬 제목을 렉트 폭으로 재는 자리다).
  <b><c>Help_Root</c> 는 0건</b>.
* 저장된 씬에서 렉트를 다시 읽어 손으로 검산했다 — 도움말 창의 <b>모든</b> 자식이
  안전 영역 안이다(창 안전 x 493~1427 · y 237~842 / 상세 속판 안전 x 824~1414 · y 338~798).
* ★★★ <b>플레이 모드 실측</b> — <c>LastSanctuary/도움말/백과 열기 (시험)</c> 로 창을 띄우고
  복제된 탭 여섯을 확인했다:
  <c>HelpTab_1 → Btn_Tab_On · color (1,1,1,1)</c> / <c>HelpTab_2 → Btn_Tab_Normal · color (1,1,1,1)</c>.
  <b>색이 흰색</b>이라는 것이 66-3 ②가 실제로 고쳐졌다는 증거다.
* 플레이 중 콘솔 <b>에러 0</b>. 조언 카드·짚어 주기도 띄워 봤다(둘 다 크기가 바뀐 판이다).

★ <b>「백과 열기 (시험)」 메뉴를 새로 만들었다</b>(<c>HelpMenu.cs</c>) — 이 메뉴 묶음에는
  「조언 카드」와 「짚어 주기」 시험은 있는데 <b>정작 백과를 여는 길이 없었다</b>. UI 를 손볼 때
  «지금 이 창이 어떻게 보이는가» 를 바로 봐야 한다.

---

### UI-66-5. ★★ 유물 아이콘 — <b>기계가 못 잡는 「닮음」</b> 여덟 짝

<c>Tools/check_relic_icons.py</c> 는 «이상 없음» 이었다 — <b>같은 칸·같은 파일을 두 유물이
쓰는 일</b>은 없었다. 그런데 유저가 본 것은 그것이 아니라 <b>«칸은 달라도 그림이 같은 소재»</b>
였다. 66칸을 <b>한 장에 늘어놓고</b> 눈으로 훑어 짝을 찾았다.

| 유물 | 전 | 후 | 왜 |
|---|---|---|---|
| 700005 마른 각질 | sheet4_0308 | <b>sheet2_0201</b> | 720019 돌아갈 곳의 기억과 «붉은 인장 찍힌 양피지» 가 같았다 → 갈라진 마른 돌판 |
| 700011 들뜬 손톱 | sheet4_0302 | <b>sheet2_0401</b> | 700008 저린 손끝과 «피 묻은 해골 손» 이 각도만 달랐다 → 피 묻은 발톱 |
| 710010 부푼 림프절 | sheet4_0405 | <b>sheet4_0608</b> | 700003 부어오른 자리와 «고리 세포 뭉치» 가 같았다 → 붉게 부은 쪽(염증이니 붉은 것이 맞다) |
| 710012 굳은 혈병 | sheet4_0605 | <b>sheet4_0204</b> | 710004 비만세포 주머니와 «붉은 구슬 항아리» 가 같았다 → 엉긴 적혈구 |
| 710014 굶주린 포식포 | sheet4_0101 | <b>sheet4_0202</b> | 710018 굳어진 항원과 «붉은 속의 창백한 가시구» 가 겹쳤다 → 입이 열린 세포(누런 초록) |
| 710015 보체의 그물 | sheet3_0404 | <b>sheet4_0606</b> | 710002 보체의 사슬과 «사슬 고리» 가 같았다 — 이름이 짝이라 그림까지 짝이면 못 고른다 |
| 720007 검은 숲의 홀씨 | sheet4_0108 | <b>sheet1_0505</b> | 700012·710018 과 가시구가 겹쳤다. <b>보스 전리품은 한눈에 달라야</b> 한다 → 초록 홀씨 |
| 720021 묻힌 심장 | sheet3_0206 | <b>sheet3_0603</b> | 720018 스스로 메운 살과 «금테 두른 붉은 심장» 이 같았다 → 바위에 박힌 붉은 결정 |

★ <b>«어느 쪽을 바꿀지» 는 이름이 정했다.</b> 지도는 「돌아갈 곳의 기억」이 맞고 사슬은
  「보체의 사슬」이 맞다 — 이름과 그림이 정확히 맞는 쪽을 남기고 <b>나머지를</b> 옮겼다.
⚠ 남는 «닮음» 이 아직 있다: <b>플라스크 다섯</b>(맑은 진물·첫 재채기·굳은 침·식은 땀·서늘한 해열)이
  실루엣을 나눠 쓴다. 색이 빨강/보라/초록/파랑으로 확실히 갈려 화면에서는 구별되므로 <b>일부러 두었다</b> —
  «물약 계열» 로 읽히는 편이 오히려 낫다.

### UI-66-6. ★ 아이콘을 「두 벌로 적어 두었던」 것 셋

셋 다 <b>다음 사람이 밟을 지뢰</b>였다. 고쳤다.

**① <c>gen_relic_table.py</c> 를 돌리면 아이콘 배정이 통째로 날아갔다.**
  아이콘 키가 두 곳에 있었다 — <c>gen_relic_table.RELICS</c> 의 열두 번째 칸(<c>relic_scab</c> 같은
  <b>임시 이름</b>)과 <c>assign_relic_icons.ICONS</c>(볼트에서 자른 <b>실제 원화 칸</b>). 흐름이
  «표 생성 → 아이콘 배정» 순이라, <b>표 생성기를 나중에 한 번만 돌리면</b> 66종이 임시 이름으로
  되돌아간다. 밸런스 한 줄 고치러 왔다가 아이콘을 날리는 것이다.
  → <c>apply_icon_keys()</c> 를 더해 <b>표를 쓰기 전에 배정표를 덮어씌운다</b>.
    이제 <b>어느 순서로 돌려도</b> 같은 값이 된다(<c>gen_relic_assets.py</c> 가 «이미 그림이
    있으면 안 덮는다» 로 같은 종류의 사고를 막은 것과 같은 처방).

**② <c>slice_relic_icons.py</c> 가 164칸을 <b>전부 <c>Resources/</c> 에</b> 넣고 있었다.**
  <c>Resources/</c> 는 <b>통째로 빌드에 실리는 자리</b>다. 그래서 누군가 손으로 98칸을 지웠던
  모양인데(<c>.gitignore</c> 의 «유물 아이콘 «팔레트»» 항목이 그 흔적이다) — <b>손으로 한 일은
  다시 돌리면 되돌아온다</b>.
  → 이제 스크립트가 <b>스스로 가른다</b>: 쓰는 칸 66 → <c>Resources/RelicIcons/</c> ·
    나머지 98 → <c>Art/RelicIconPalette/</c>(gitignore). <b>배정에서 빠진 옛 그림은 <c>.meta</c> 까지
    지운다</b> — 안 지우면 «바꿨는데 옛 그림이 남아» 어느 쪽이 산 것인지 못 가린다(이번에 8개 치웠다).

**③ <c>assign_relic_icons.py</c> 의 백업 이름에 <b>날짜가 박혀</b> 있었다.**
  다시 돌리자 «아이콘 배정 <b>전</b>» 백업이 «이미 배정된» 상태로 덮어써졌다(실측).
  → 이름을 <b>돌리는 날짜</b>로 짓는다.

### UI-66-7. ⚠⚠ 되돌려 놓은 것 — <b>발굴 밸런스가 표에만 있었다</b>

<c>gen_relic_table.py</c> 를 돌리자 <c>DigOutcome</c> 시트 13칸이 <b>옛 값으로 되돌아갔다</b>.
UI-65 의 발굴 밸런스가 <b>xlsx 에만</b> 들어가고 스크립트에는 반영이 안 돼 있었던 것이다 —
그 표의 <c>Info</c> 시트가 <b>이미 경고하고 있던 그대로</b>다
(*«이 표는 그 스크립트가 통째로 다시 씁니다 — 표를 손으로 고쳤다면 스크립트에도 같은 값을 반영할 것»*).

→ xlsx 의 값을 <b><c>DIG_OUTCOMES</c> 로 옮겨 적고</b> 다시 돌려 원상복구했다.
  검산 — 백업본과 칸 단위로 견줘 <b><c>Relic</c> 시트의 아이콘 8칸 말고는 전부 같음</b>
  (<c>Grade·EffectType·DigOutcome·Drop·DigChoice·Dialogue</c> 모두 0건),
  <c>RelicDigTable.asset</c> 도 커밋 상태로 돌아왔다.
★ 남는 값: 에너지 24/40 · 큰 에너지 3/100 · 회복 12/25 · 피해 13/22 · 침식+ 12/14 ·
  침식− 6/10 · 꽝 12 · 일반 14 · 레어 3 · 에픽 1 (합 100).

### UI-66-8. 표에 규칙을 적었다

유물 표 <c>Info</c> 시트에 <b>「아이콘 고르기 — «겹치면 안 되는 것» 이 둘입니다」</b> 절을 신설했다
(생성기 <c>INFO</c> 에 넣었다 — 그 시트는 스크립트가 통째로 다시 쓰므로 손으로 적으면 날아간다).
① 같은 칸 금지(기계가 막는다) · ★★ ② <b>«닮은» 것도 금지(기계가 못 잡는다)</b> ·
새 유물을 더할 때는 <b>이미 쓰는 66칸을 한 장에 늘어놓고</b> 고를 것 · 남는 98칸이 어디 있는지 ·
<b>표 생성기를 돌려도 icon 칸은 안 되돌아간다</b>는 사실.

### 확인된 것

* <c>recompile_scripts</c> <b>에러 0 · 경고 0</b>, 플레이 중 콘솔 <b>에러 0</b>, Assets/Refresh 후 <b>에러 0</b>.
* <c>check_relic_icons.py</c> — ① 키 중복 없음 ② 그림 파일 중복 없음 ③ 빈 키 없음 ④ 모든 키에 PNG 있음.
* 표 실측 — 66행 전부 <c>sheet_*</c> 키, 빈 칸 0. 바꾼 8종이 표·에셋에 그대로 들어갔다
  (<c>git diff</c> 로 <b>정확히 유물 에셋 8개만</b> 바뀐 것을 확인).
* 도움말 세 창이 <b>꺼진 채로</b> 저장된 것 확인(<c>activeSelf: false</c> 셋).

### 아직 확인 못 한 것 (유저가 볼 것)

1. ★★ <b>밝기가 «충분히» 밝은지</b>는 눈으로 볼 것. 한 단(<c>#3D5468</c>)이 지금 값이고
   두 단은 흰 글자가 묻혀서 되돌렸다. 더 밝히고 싶다면 <c>ui_make_tab_sprites.py</c> 의
   <c>JOBS</c> 에서 단수를 올리되 <b>글자 색을 같이 어둡게</b> 해야 한다.
2. 창이 <b>980×648</b> 로 커졌다. 1920×1080 에서는 넉넉하지만 창을 끌어다 놓은
   자리에 따라 처음 느낌이 다를 수 있다(<c>UiWindowDrag</c> 는 그대로다).
3. <b>바뀐 아이콘 여덟이 유물의 «결» 에 맞는지</b> — 특히 700005 마른 각질(돌판)과
   720021 묻힌 심장(바위 속 결정)은 <b>글자 그대로의 그림이 아니라 은유</b>다.
   어색하면 <c>Art/RelicIconPalette/</c> 의 98칸에서 골라 <c>assign_relic_icons.py</c> 만 고치면 된다.
4. ⚠ <b>이번 작업과 무관한 것 하나</b>: <c>[Minimap] HUD_Minimap/DigMarks 를 찾지 못했습니다</c>
   경고가 판마다 뜬다. <c>MinimapPanel</c> 이 찾는 <c>DigMarks</c>·<c>MarkTemplate</c> 이
   <b>씬에 아예 없다</b>(<c>Tools/</c> 의 어떤 스크립트도 만들지 않는다) — UI-63 의
   «미니맵 느낌표» 가 <b>한 번도 그려진 적이 없다</b>는 뜻이다. 별도 작업으로 남긴다.

### 씬 변경 여부

**있음.** <c>Help_Root</c> 아래 세 창의 크기·좌표 전면 조정 + 그림 배선(배선 메뉴가
<c>UI_Root</c> 쪽도 다시 칠한다). <c>save_scene</c> <b>1회</b>.

### 씬반영요청 목록

- 없음 (MCP 로 좌표를 넣고 에디터 메뉴로 그림을 깔았다)

---

## UI-67. ★★★ 경제 착지점 «12명 Lv35» · 만렙 40 · <b>유물 3칸</b> · 로그 50줄 스크롤 · 중대 사건 배너 · ★★ AI 충돌 둘 (2026-08-26, 24차)

### 무엇을 / 왜

유저 지시 아홉 건이 한 번에 들어왔다.

| # | 지시 | 결과 |
|---:|---|---|
| 1 | 경제 밸런스 **12명 35lv 기준**으로 재밸런싱 (중대 변경은 질문 · 값은 표에 반영) | ✅ 질문 넷으로 확정 → 중립/에픽 보상만 상향 (UI-67-1) |
| 2 | **만렙 40 LV** | ✅ `CharacterUpgradeService.maxLevel` 신설 (UI-67-2) |
| 3 | 각성 15LV 부터여야 하는데 **적용 안 됨** | ✅ ★★ **판정은 멀쩡했고 «표시» 가 안 보고 있었다** (UI-67-3) |
| 4 | 유물 장착 **3칸** · 초상화UI(아이콘만) · 로스터 연동 | ✅ 장부·효과·창 셋 다 (UI-67-4) |
| 5 | 허드 액션 버튼 첫 클릭에 도움말이 **안 나온다** | ✅ ★★ **기능은 멀쩡했다** — 되돌릴 길이 없었을 뿐 (UI-67-5) |
| 6 | 로그 **50개**까지 보관 · 올려 볼 수 있게 스크롤바 | ✅ (UI-67-6) |
| 7 | **AI** — 원거리가 지원을 안 온다 · 전방이 혼자 나가 죽는다 | ✅ ★★ 충돌 <b>둘</b>을 찾았다 (UI-67-7) |
| 8 | 중대 사건 **페이드 인 텍스트** (미니맵–허드 액션 사이) | ✅ (UI-67-8) |
| 9 | 도움말에 **정신 이상·능력치 상세** 별도 항목 | ✅ (UI-67-9) |

---

### UI-67-1. ★★★ 경제 — 착지점을 «w30 = 12명 Lv30» 에서 «Lv35» 로

**먼저 모델을 되살렸다.** 136절의 계산 스크립트는 스크래치패드에 있어서 남아 있지 않았다.
그래서 「웨이브 부하」 시트의 30행을 입력으로 놓고 **같은 순서로 다시 돌려** 재현했다 —
① 충원 ② 남는 에너지를 **최저 레벨** 캐릭터에게 ③ 그 상태를 기록 ④ 그 웨이브를 치르며 번다.
30행이 전부 **±0.3 Lv** 안에 들어왔다(그래서 이것이 그때의 모델과 같다고 본다).
그 모델은 이제 **`Tools/table_update_20260826_econ_lv35.py` 안에 남는다** — 또 잃지 않는다.

**유저 확정 넷** (질문으로 받았다)

| 물음 | 확정 |
|---|---|
| Lv30 부터의 강화 «벽»(등비 1.35)을 35 로 옮길까 | **Lv30 에 그대로 둔다** |
| 파티가 세지면 몬스터도 다시 잡을까 | **몬스터는 그대로** — 기획서가 «Lv35 3부대면 무난» 이라 정의한 그 상태가 된다 |
| 모자란 수입을 어디서 | **중립·에픽 보상만** |
| 유물 3칸이면 효과가 3배인데 | **수치 그대로 — 3배 그대로** |

**필요한 것** — 12명 × Lv35 = 강화비 **107,280** + 생성비 5,800 = 113,080.
지금 수입 77,840(웨이브 처치 17,920 + 중립 59,920) → **중립을 ×1.81** 해야 한다.

★★★ **구간 배율을 넣는 장치를 만들지 않았다.** 중립 표에 이미 **등장 «거리»** 가 있다:

```
  1001 종양 거미   15~99 타일    초반부터 닿는다     ->  유지 (초반을 안 건드린다)
  1002 종양충      100~199       부대를 꾸리면        ->  x1.3   26~44 -> 34~57
  1004 고르도네    100~199                            ->  x1.3
  1003 종양 두더지 200~320       원정을 보내야        ->  x3.0  200~330 -> 600~990
  1101~1103 에픽   200~320       요구 Lv 10/15/20     ->  x3.0  600~900 -> 1,800~2,700
  1104 폴리르      200~320       요구 Lv 25           ->  x3.0  900~1,200 -> 2,700~3,600
```

**먼 종만 올리면 후반에만 수입이 오른다.** 밸런스 기획서의 «초반은 처치만으로 충분 /
중반은 적극적인 중립 사냥 / 후반은 에픽 보상을 적극 활용» 이 정확히 이 모양이다.
역산한 구간 배율(1~10 ×1.00 · 11~15 ×1.15 · 16~20 ×1.30 · 21~25 ×1.55 · 26~30 ×2.33)에
위 종별 배수를 사냥 구성에 얹으면 **오차 ±10%** 로 들어맞는다.

**모델 결과**

| 웨이브 | 5 | 10 | 15 | 20 | 25 | 30 |
|---|--:|--:|--:|--:|--:|--:|
| 기획서 요구 평균 Lv | 5 | 10 | 13 | 18 | 23 | (30) |
| 전 (136절) | 4.8 | 9.4 | 11.5 | 19.0 | 21.0 | 29.8 |
| **후** | **4.8** | **9.4** | **11.9** | **20.4** | **24.4** | **35.0** |

★ **w30 에 주력 능력치가 정확히 100(상한)에 닿는다** — 136-0절 1번이 «Lv35 에 주력 상한» 으로
잡아 둔 그 지점이 이제 **경제로도 닿는 곳**이 됐다. 그 위 Lv36~40 은 유물·에픽 보상의 몫이다.

⚠ **초반 다섯 웨이브는 한 톨도 안 변한다**(1001 을 안 건드렸다). 중반이 조금 넉넉해지고
  (w20 19.0 → 20.4) 후반이 크게 열린다 — 몬스터를 그대로 두기로 했으므로
  **20·25 보스는 그만큼 쉬워진다**(기획서 요구선보다 2 Lv 위다).

**표에 반영한 것** (유저 지시: *«변경한 데이터 값은 테이블에 반드시 반영»*)

| 파일 / 시트 | 무엇 |
|---|---|
| `임시용 중립 몬스터` `neutrality_mon` | **14칸** — 7종의 min/max_energy |
| `능력치 및 공식 정리` `웨이브 부하` | **9열 × 30행** — 평균 Lv · 주력/일반 · 캐릭터 HP · 몇 대에 죽나 · 한 대÷HP · 파티 DPS · 잡몹 부하 · 중립 수입 + 머리말에 개정 한 줄 |
| `능력치 및 공식 정리` `계수` | **5행** 덧붙임 — 만렙 40 · 유물 칸 3 · 각성 최소 Lv 15 · 중립 수입 배율 |

⚠ `계수` 는 **행을 끼워넣지 않고 맨 아래에** 붙였다 — 다른 시트가 절대 행 번호로 참조한다(38-2절).
⚠ **Excel COM 으로 썼다** — openpyxl 로 저장하면 하이퍼링크·주석·수식 캐시가 날아간다(136-4절).
백업: `데이터 테이블/_백업/20260826_econ_lv35/` (11개).
`sync_tables_to_assets.py` 실측 — **바뀐 에셋은 정확히 중립 7개, 14칸**(`git diff` 로 확인).

⚠⚠ **「웨이브 부하」 의 세 열은 «맞춘 곡선» 이다.** 원래 계산이 남아 있지 않아 옛 30행에서 되짚었다 —
  `파티 DPS`(2차식 · 오차 ±5%) · `잡몹 부하`(실효 화력 0.647 — 옛 행 전부에서 같은 값이 나왔다) ·
  `한 대÷캐릭터 HP`(**방어력 상승을 빼고 보수적으로** 잡았다 — 실제로는 이보다 덜 아프다).
  다음에 이 시트를 고칠 사람은 이 사실을 알고 시작할 것.

---

### UI-67-2. 만렙 40

`CharacterUpgradeService.maxLevel` 신설(기본 40 · 0 이면 만렙 없음).

★ **막는 곳은 이 서비스 하나다** — 버튼 잠금(`CharacterGrowthPanel`)도 `CanUpgrade` 를 읽으므로
**한 줄이면 화면과 처리가 같이 닫힌다**. 소환수 금지가 이미 그 방식이다.
⚠ **정신 이상 「고조」의 무료 강화도 막힌다**(`GrowFree`) — «자원을 안 쓴다» 가 «천장을 넘는다» 는 뜻이 아니다.
⚠ **비용 곡선으로 천장을 흉내 내지 않았다** — 등비는 값이 커질 뿐 끝이 없고, `CostForLevel` 의
  주석이 이미 «Lv90 쯤에서 int 를 넘겨 **음수 비용**이 된다(가장 나쁜 방향의 고장)» 을 적어 두었다.

성장 창에 **「만렙」** 표시를 따로 뒀다 — «능력치 상한»(12종이 전부 100)과 **다른 사건**이고,
실제로 상한에 안 닿은 채 만렙에 먼저 닿는 캐릭터가 있다(성장 유형 밖 능력치).

---

### UI-67-3. ★★ 각성 «15Lv» — 판정은 멀쩡했고 <b>표시</b>가 안 보고 있었다

유저 리포트: *"각성 15LV 부터 가능해야 하는데 적용 안됨(더 낮은 레벨에도 각성 가능)"*.

**실제로 각성은 막혀 있었다.** `HeroAwakeningService.TryAwaken` 이 `awakenMinLevel`(씬 실측 **15**)을
보고 있고, 회복 쪽 `TryAwakenByHeal` 도 같은 값을 본다.

**막히지 않은 것은 화면이었다.** `StateOf` 가 **처치 수만** 보고 `HeroState.Ready`(★ 각성 가능)를
돌려줘서, **Lv5 짜리 캐릭터에도 «각성 가능» 이 떴다.** 규칙이 판정과 표시 두 곳에 갈려 있었다.

→ `StateOf` 에 레벨 조건을 **같이** 넣고, «눈금은 찼지만 레벨이 남았다» 에 **이름을 줬다**
(`HeroState.LevelLocked`). 그냥 `None` 으로 되돌리면 «다 모았는데 왜 아무 표시가 없나» 가 된다.
성장 창은 그 상태에 **「· Lv.15 부터 각성」** 을 흐린 색으로 적는다.

---

### UI-67-4. ★★★ 유물 장착 1칸 → <b>3칸</b>

| 고친 곳 | 무엇 |
|---|---|
| `RelicInventory` | 사전의 **값을 배열로**(`Dictionary<int,int[]>`). 열쇠는 그대로 캐릭터 정의 ID. `equipSlots` 신설 |
| `RelicEffectService` | `_worn` 을 **목록**으로 · `Applied.owner` 신설 · `ValueOf` 는 **세 칸을 더한다** |
| `RelicPanel` | «그 캐릭터가 이 유물을 꼈나» 로 판정 · 빈 칸이 있어야 낀다 · 안내에 «유물 칸 2/3» |
| `UnitPortraitPanel` | ★ **아이콘만** (이름 칸은 끈다) · 아이콘 셋 |
| `CharacterRosterPanel` | 행에 아이콘 셋 |
| `CharacterGrowthPanel` | 낀 것 **전부**를 등급 색으로 이어 적고 효과도 전부 |
| `RelicIconStrip` (신설) | 원본 아이콘 하나를 받아 **칸 수만큼 복제**해 늘어놓는다 |

★★★ **`RemoveAllFor(unit)` 가 가장 위험한 자리였다.** 칸이 하나였을 때는 벗을 때 그 캐릭터의
보정을 통째로 떼면 됐다. 칸이 셋이면 그 한 줄이 **같이 끼고 있던 나머지 둘의 보너스까지 지운다** —
«유물 하나를 벗었더니 능력치가 세 개만큼 빠지는» 고장이다. 그래서 `Applied` 에 **붙인 주인**을
적어 두고 `RemoveOwnedBy(unit, relic)` 로 **그 유물 것만** 뗀다. 「두꺼워진 가피」의 문턱형도
같은 이유로 유물마다 따로 켜고 끈다.

★ **`ValueOf` 는 «더한다»** (흡혈 5% 짜리 둘 = 10%) — 유저 확정 «3배 그대로».
  ⚠ 다만 **짝(v1,v2)은 안 더한다**(`TryValueOf`) — v2 는 «문턱»·«초»·«상한» 이라 더하면 뜻이 깨진다
  (체력 30% 아래 + 50% 아래 = 80% 아래가 아니다). **v1 이 가장 큰 하나**가 이긴다.

★ **세이브 형식은 한 칸도 안 바뀐다** — `CaptureEquipped` 가 «한 칸 = 한 쌍» 으로 내보내므로
  캐릭터마다 쌍이 여러 개 나올 뿐이다. **옛 세이브(캐릭터당 한 쌍)는 자연히 첫 칸으로 들어온다.**

★ **아이콘은 «복제» 한다** — 로스터 행·상세 카드에 이미 아이콘 하나가 씬에 있고 그 좌표는
  실측으로 잡힌 것이다. 씬에 둘을 더 손으로 만들면 **세 곳의 좌표가 따로 논다**.
  원본을 «칸 0» 으로 두고 나머지를 코드가 옆에 복제하면 자리 규칙이 한 곳에만 있다(§10 H-2 와 같은 결).
  ⚠ 복제는 **한 번만** 한다 — 갱신마다 하면 로스터가 0.2초마다 GC 를 돌린다.

⚠⚠ **로스터 행에 `RelicIcon` 이 씬에 아예 없었다.** 진행상황 168절이 «얼굴 60×60 + 액자 +
  유물 아이콘 26px» 로 행을 재배치했다고 적어 두었는데, **이 프로젝트의 씬에는 그중 아무것도 없다**
  (`RelicIcon`·`PortraitFrame`·`SquadTab` 셋 다 0건 · 행은 340×78 이고 168절은 380×90 이라 한다).
  볼트 문서가 씬보다 앞서 있는 상태다. → 이번에 **행 오른쪽 아래에 20px 아이콘 하나를 새로 만들고**
  침식 바를 324 → 252 로 줄여 자리를 냈다(x 264~332 가 아이콘 셋의 자리다).
  ★ 얼굴·액자·부대 띠는 **손대지 않았다** — 이번 지시 밖이고, 168절과 실제 씬의 차이는 유저가 볼 일이다.

---

### UI-67-5. ★★ 도움말이 «첫 클릭» 에 안 뜨던 것 — <b>기능은 멀쩡했다</b>

배선을 전부 되짚었다. **어디에도 이상이 없었다** —
표에 `ActionCreate`(24) ~ `ActionSettings`(30) 일곱이 다 들어 있고, `HelpTrigger` enum 과 번호가 맞고,
`ActionPanel`·`UpgradeButtonUI` 가 `InterceptFirstUse` 를 부르고, `HelpCardPanel` 도 씬에 있다.

**PlayerPrefs 를 직접 읽어 보고 답이 나왔다** (`HKCU\Software\DefaultCompany\Last Sanctuary`):

```
help_nexus|help_wave|help_speed|help_energy|help_hit|help_neutral|help_relic_equip|
help_event|help_upgrade|help_save|help_battle|help_erosion|help_create|help_squad|
help_tactics|help_boss|help_epic|help_awaken|help_mental_error|help_enrage|help_rally|
help_death|help_relic_dig
```

**일곱 개가 전부 «이미 읽음» 으로 적혀 있었다.** 그러니 `InterceptFirstUse` 는 규칙대로 `false` 를
돌려주고 버튼은 평소대로 동작한다 — 고장이 아니라 **설계대로**다(문명과 같이 판을 다시 시작해도
기억이 남는다).

⚠ **진짜 구멍은 여기였다** — `HelpService.ForgetAll()` 의 주석은 *«환경 설정에서 부른다»* 라고
적혀 있는데 **실제로는 어디서도 부르지 않았다**(에디터 메뉴 하나뿐). 즉 **게임 안에서 되돌릴
방법이 없었고**, 그래서 «안 나온다» 를 확인할 길조차 없었다.

→ 환경 설정에 **「도움말 다시 보기」** 버튼을 신설했다(`HUD_Settings/Body/HelpResetButton` —
단축키 버튼을 복제해 그 줄을 반씩 나눠 썼다. 칸을 새로 만들면 창 높이를 다시 잡아야 한다).
→ 이번 세션에서 **그 키를 실제로 지웠다**(백업: 스크래치패드 `help_seen_backup.bin`).
  다음에 판을 켜면 일곱이 다시 뜬다.
...
`HUD_Log` 를 로스터·유물 창과 **같은 구조**로 맞췄다(135-3절이 세운 모양 그대로):

```
HUD_Log
 |- Title
 |- ScrollView   ScrollRect (vertical · Clamped · sensitivity 24)
 |   \- Viewport  Image(알파 0) + RectMask2D
 |       \- Lines  VerticalLayoutGroup + ContentSizeFitter   <- 옮겨 왔다
 |- Scrollbar    Image + Scrollbar(BottomToTop · size 0.2)
 |   \- Handle    Image
 \- LineTemplate (그대로 · 비활성)
```

- `maxLines` 10 → **50**. ★ **줄 재사용은 그대로다** — 50줄이 되어도 `Instantiate` 는 최대 50번뿐이다.
- **새 줄이 붙으면 맨 아래로 따라 내려간다.** ⚠ 유저가 **위로 올려 둔 동안에는 따라가지 않는다** —
  읽고 있는 자리를 뺏지 않으려는 것이다(`IsAtBottom` · 여유 2%).
- ⚠ 줄을 붙인 «그 프레임» 의 content 높이는 아직 옛값이라 `ForceRebuildLayoutImmediate` 뒤에 내린다.
- ⚠ 경로는 `ScrollView/Viewport/Lines` 를 먼저 보고 **옛 경로(`Lines`)를 폴백으로 남겼다**.

---

### UI-67-7. ★★★ AI — 규칙 <b>둘</b>이 서로를 밀어내고 있었다

**① 「동료 구원 거리 12」가 한 번도 12로 동작한 적이 없다.**
`UnitCombat.FindAllyAttacker` 의 마지막 거르기가 `EffectiveDetectRange` **하나만** 봤다.
캐릭터 템플릿 실측은 `detectRange 6` · `allyCallRange 12` 다 — 즉 12타일 떨어진 동료가 맞고 있어도
**때리는 적이 6타일 밖이면 걸러졌다.** 인스펙터의 «12» 가 조용히 «6» 으로 동작하고 있었던 것이다.
→ **둘 중 큰 쪽**을 쓴다. 반격·동료 구원이 목줄 관문에서 이미 `max(leashRange, …)` 로 다뤄지는
그 규칙과 **같은 모양**이고, 달려간 거리는 그 관문이 **귀환 지점 기준**으로 다시 자른다.
⚠ 안 보이는 적은 여전히 거른다(`IsFogVisible`).
⚠ 이 함수는 **몬스터·중립도 쓴다** — 그쪽도 «동료가 맞으면 자기 `allyCallRange` 만큼 반응» 으로 같이 바뀐다.

**② 전방이 나가 서는 자리가 곧 «지원이 안 닿는 자리» 였다.**
가로막기(`TryPickInterceptSpot`)는 **구역 밖**(경계 + 여유 1.5)에 서고 목줄까지 +4 늘린다.
그런데 지원(`TryPickSupportSpot`)은 `IsInsideZone(…, halfExtent)` — **구역 «안» 의 적만** 봤다.
**두 규칙이 서로를 밀어내고 있었고, 그 사이가 벌어질수록 전방은 혼자가 됐다.**
→ `supportOutsideZone`(기본 8타일) 신설. 조건을 **없애지 않고 구역을 그만큼 넓혀서** 본다 —
없애면 «지나가는 적을 쫓아 구역을 비운다» 가 되돌아온다(그 함수가 원래 막으려던 것이다).
⚠ 지원 자리의 **목줄도 같은 «넓힌 구역»** 을 쓴다. 자리는 밖인데 목줄만 안이면 도착하자마자
  목줄 관문이 그 적을 잘라내 **«가서 가만히 서 있는»** 상태가 된다.

**③ 전방은 «혼자» 이거나 «떼 앞» 이면 안 나간다.**
`frontInterceptAllyRange`(12타일 안에 아군이 없으면) · `frontInterceptSwarmCount`(판정 거리 안의
웨이브 몬스터가 4 이상이면) 둘 중 하나면 **구역 경계 안쪽(0.8배)** 에서 맞이하고 **목줄 보너스도 안 받는다**
(보너스를 그대로 주면 물러선 자리에서 다시 그만큼 나가 버려 판정이 무의미해진다).
⚠ **«가로막기를 그만둔다» 가 아니다** — 그 적을 향해 서는 것은 그대로다. 완전히 손을 떼면
  전방이 순찰로 돌아가 뒤가 더 위험해진다.
★ «적극적으로 방어한다» 는 원래 지시를 **되돌리는 것이 아니다** — 동료가 곁에 있으면 예전과 똑같이 나간다.

---

### UI-67-8. 중대 사건 배너

`UI_Root/HUD_Notice` 신설 — 미니맵 위·허드 액션 아래의 빈 띠(우측 y 338~604) 한가운데,
520×64, 오른쪽 정렬. **나타남 0.35초 → 머묾 2.4초 → 사라짐 0.7초** + 아래에서 14px 떠오른다.

★ **«무엇이 중대한 사건인가» 를 배너가 정한다.** 유물·전투 코드에 `HudNotice.Show(...)` 를 심지
않는다 — 그러면 그 시스템들이 배너를 **알아야** 하게 된다. 대신 **이미 있는 public 이벤트**에 붙는다
(`HelpService` 가 튜토리얼에서 택한 그 방향이고, 여기서도 그대로 맞다):

- `RelicInventory.OnGranted`(신설) — 유물을 **새로** 얻었다. ★ 주는 통로가 넷이라 **«들어오는 문»** 하나에 붙는다.
- `DamageableUnit.OnAnyDied` — 에픽 중립 토벌 · 웨이브 보스 격파. **잡몹은 안 알린다** (쉬지 않고 뜨면 «중대» 가 아니게 된다).

⚠ **`unscaledDeltaTime` 으로 흐른다** — 도움말 카드가 게임을 멈춰도 배너는 나타났다 사라져야 한다.
⚠ **클릭을 먹지 않는다**(`blocksRaycasts = false`) — 미니맵과 허드 액션 사이라 레이캐스트를 받으면 둘을 가로막는다.
⚠ **로그를 대신하지 않는다** — 같은 사건이 `HudLog` 에도 남는다(이제 50줄을 거슬러 볼 수 있다).

---

### UI-67-9. 도움말 — 「능력치 낱낱」·「정신 이상 낱낱」

★ **«별도» 로 만든 이유** — `help_stats`·`help_mental_error` 는 **조언 카드로도 뜨는** 항목이라
서너 줄이 한계인데, 유저가 원한 것은 «열두 칸이 각각 무엇을 정하는가» · «열한 가지가 각각 무엇을
하는가» 라는 **목록**이다. 한 항목에 다 넣으면 카드가 넘치고, 카드에 맞춰 줄이면 목록이 안 된다.
→ **백과에만 있는 항목**을 옆에 세웠다(`trigger` 를 비우면 저절로 안 뜬다 — `help_camera` 가 그 모양).

★ 「함께 볼 것」은 한 칸뿐이라 **사슬로** 이었다:
`help_stats → help_stats_detail → help_upgrade` · `help_mental_error → help_mental_detail → help_erosion`.

문구는 **표가 정본**이고(`help_string_merge.py`) 어투는 기존 도움말과 맞췄다(문어체 «~습니다» ·
짧은 평서문 · 강조는 `<b>`). 수치는 코드·표에서 확인한 것만 적었다 —
상한 100 · 치명타 1.5배 · **명중·크리는 원거리 전용** · **저항력은 강화로 안 오른다** ·
정신 이상 열한 가지(40001~40011).

스트링 키 **6개** 추가(1,032 → 1,036). 잃은 키 **0**(옛 키 전부가 새 표에 있는지 세어 확인).

---

### 확인된 것

* `recompile_scripts` **에러 0 · 경고 0** (다섯 번) · 콘솔 **에러 0** · `Assets/Refresh` 후 **에러 0**.
* 씬 저장 **3회** (매번 Edit 모드인 것을 확인).
* 새 씬 오브젝트가 저장 파일에 실제로 들어간 것을 YAML 로 확인 —
  `HUD_Notice` 1 · `HelpResetButton` 1 · `RelicIcon` 1 · `ScrollView`/`Scrollbar` 각 4(로스터·유물·도움말·로그).
* 표 → 에셋 반영 실측 — `git diff` 에 **중립 7개 파일 · 14칸만** 바뀌었다(나머지는 내용 동일).
* 스트링 키 잃은 것 0 (`comm` 으로 대조).
* 도움말 에셋 검산 통과 — 항목 28개 · 계기 23 · 백과 전용 5 · «한 UI 안» 규칙 통과.
* 폰트 메뉴 실행 — 새 TMP **1개**(배너 라벨)에 네오 둥근모 적용.

### 아직 확인 못 한 것 (유저가 볼 것)

1. ★★★ **밸런스는 전부 계산 모델이다.** 플레이로 확인할 것 — 특히 «중립 1003·에픽을 실제로 그만큼
   사냥할 수 있는가». 모델은 **사냥할 수 있다고 가정**한다. 못 하면 Lv35 에 못 닿는다.
2. ★★ **20·25 웨이브 보스가 쉬워진다** — 몬스터를 그대로 두기로 했으므로 파티가 기획서 요구선보다
   2 Lv 위로 간다(w20 20.4 vs 18 · w25 24.4 vs 23). 너무 싱거우면 그때 보스만 다시 잡을 것.
3. ★★ **유물 3칸의 체감** — 효과가 그대로 3배다. 특히 흡혈·반사처럼 **더해지는** 것이 셋 겹칠 때.
4. ★ **로스터 행의 유물 아이콘 셋**(x 264~332)이 침식 바(8~260)와 안 겹치는지 · 20px 이 너무 작지 않은지.
5. ★ **배너 자리**(우측 y 338~604 한가운데) — 허드 액션의 높이는 버튼 수에 따라 자동으로 변한다.
   버튼이 하나 더 늘면 배너와 겹칠 수 있다.
6. ★ **AI 세 값**(`frontInterceptAllyRange 12` · `frontInterceptSwarmCount 4` · `supportOutsideZone 8`)은
   **눈으로 보고 맞춰야 하는 값**이다. 전방이 너무 소극적이면 앞의 둘을 낮추고, 지원이 너무 멀리 나가면 뒤를 낮춘다.
7. ⚠⚠ **볼트 168절과 실제 씬이 어긋나 있다**(UI-67-4 의 ⚠⚠). 로스터 행의 얼굴·액자·부대 띠가
   **씬에 없다**. 다른 PC 의 작업이 안 넘어온 것인지 문서가 앞선 것인지 확인이 필요하다.
8. ⚠ **이번 작업과 무관한 것**: `[Minimap] HUD_Minimap/DigMarks 를 찾지 못했습니다` 경고가 여전하다
   (UI-66 의 «아직 확인 못 한 것» 4번 그대로).

### 씬 변경 여부

**있음.** `HUD_Log`(ScrollView·Viewport·Scrollbar·Handle 신설 + Lines 이동) ·
`HUD_Notice`(+Label) 신설 · `HUD_Settings/Body/HelpResetButton` 신설(HotkeyButton 반폭) ·
`HUD_Roster/RowTemplate/RelicIcon` 신설(ErosionBack 폭 축소) ·
`GameSystems`(maxLevel 40 · equipSlots 3) · `Character_Template`(AI 세 값). `save_scene` **3회**.
...
## UI-68. ★★★ 부대 색이 <b>사라진 진짜 이유</b> — 「그림에 색을 곱한다」 · 로스터 행 재배치(얼굴 왼쪽 끝) · ★★ 언어 버튼에 <b>Image 가 아예 없었다</b> (2026-08-26, 25차)

### 무엇을 / 왜

유저 지시 다섯 (셋을 마친 뒤 둘이 더 왔다).

| # | 지시 | 결과 |
|---:|---|---|
| 1 | *"같은 부대 일때 같은 색으로 캐릭터 로스터에 묶어두는 기능 없어짐"* | ✅ ★★★ **코드도 씬도 멀쩡했다** — 색을 «어두운 그림에 곱하고» 있었다 (UI-68-1) |
| 2 | *"캐릭터 로스터에서 초상화를 왼쪽 끝으로 옮긴 다음 버튼을 꽉 채우게"* | ✅ 행 일곱 칸 전부 다시 잡았다 — 죽은 자리 102px 를 없앴다 (UI-68-2) |
| 3 | *"언어 변경 버튼이 없음 현재 언어만 나옴"* | ✅ ★★ 그 버튼만 **`Image` 가 없었다** — 안 보이고 안 눌렸다 (UI-68-3) |
| 4 | *"유물 칸 조금 더 크게 · 초상화를 아예 버튼 이미지 영역 밖에 자연스럽게 붙여서 더 크게"* | ✅ ★★ **목록 왼쪽에 자리를 판다** — 카드를 좁히지 않고 얼굴을 밖으로 (UI-68-4) |
| 5 | *"로비화면 환경 설정에도 환경 설정 버튼들 같이 붙여주고"* | ✅ 언어·단축키·도움말 셋 — «사람에 딸린 값» 만 (UI-68-5) |

**새 C#** — `UI/LanguageSetting.cs`
**고친 C#** — `UI/CharacterRosterPanel.cs` · `UI/HudTheme.cs` · `UI/SettingsPanel.cs` · `UI/LobbySettingsWindow.cs`
**새 스크립트** — [Tools/mcp_lobby_settings_20260826.py](Tools/mcp_lobby_settings_20260826.py)
**씬 변경** — 있음(`Proto_01` 로스터 행 · 환경설정 언어 버튼 / `Lobby` 환경 설정 창 + `HUD_Hotkeys`) · `save_scene` **3회**
**새 자산** — 없음 (있는 그림만 썼다)

---

### UI-68-1. ★★★ 부대 색 — <b>「이미지에 딱 맞게」가 색을 잡아먹고 있었다</b>

**증상** — 부대를 지어도 로스터 행이 아무 색도 안 띤다. 「기능 없어짐」.

**먼저 아닌 것부터 지웠다.** 셋 다 멀쩡했다.
* `groupBySquad: 1` · `squadUsesRowTint: 1` — 씬 값 살아 있음.
* `SquadService` 씬에 있음 · `HudTheme.SquadColor()` 여섯 색 살아 있음.
* `RowTemplate/SquadTint` 오브젝트 **씬에 있음**(UI-67 다음 커밋에서 만든 것).

즉 **색을 칠하는 코드가 돌기는 돌고 있었다.** 그런데 안 보였다.

**진짜 원인 — <see cref="Image"/> 는 «그림 × 색» 으로 그린다.**

바로 앞 판(커밋 `2c8479bd`)은 유저의 *"이미지에 딱 맞게 만들어줘"* 를 받아
색 판이 카드와 **같은 스프라이트**를 쓰게 했다.

```csharp
row.SquadTint.sprite = row.Background.sprite;   // Btn_Roster_Normal
row.SquadTint.color  = new Color(청록.r, 청록.g, 청록.b, 0.22f);
```

실루엣은 1px도 안 어긋난다 — 맞다. 그런데 **색이 죽는다.**
카드 그림의 속살은 `#212B38`(33,43,56) 이다. 여기에 1부대 청록(0.35,0.85,0.95)을
**곱하면** (12,37,53) 이 나온다. 그걸 알파 0.22 로 카드 위에 얹으면

```
(33,43,56) × 0.78  +  (12,37,53) × 0.22  =  (28,42,55)
```

원래 색 `(33,43,56)` 과 **눈으로 구분이 안 되는 값**이다. 색이 «반영은 되는데 안 보인다» 가
아니라 **처음부터 회색이 나오고 있었다.** 곱셈이 원본보다 밝아질 수 없다는 것은 그 커밋의
주석이 이미 알고 있었는데(`Background.color` 곱셈을 걷어낸 이유가 그것이다),
**스프라이트를 물려받는 순간 곱셈이 되돌아온 것**을 못 봤다.

**고침 — 판은 «그림 없는 흰 판» 으로, 대신 칸을 카드 홈에 맞춘다.**

| | 앞 판 | 이번 |
|---|---|---|
| 스프라이트 | 카드 그림을 물려받음 | **없음**(흰 사각형) |
| 칸 | 행 **전체** | **카드 안쪽 홈**(x 14~371 · y 18~90) |
| 형제 순서 | **마지막**(글자 위) | **첫째**(카드 위 · 글자 **아래**) |
| 알파 | 0.22 (그 이상은 글자가 물듦) | **0.5** |

흰색에 부대 색을 곱하면 **부대 색 그대로**다. 앞 판이 «행 전체» 를 덮어 네모가 삐져나와
보였던 것은 — 그게 스프라이트를 물려받게 만든 계기였다 — 이번엔 **칸을 홈에 맞춰**
해결했다. 위·아래 테두리와 아래 빗금 레일을 비켜 가므로 삐져나올 것이 없다.

★ **형제 순서를 바꾼 것이 알파를 열어 줬다.** 유니티 UI 는 형제 순서대로 그린다 —
  **첫째 자식이 제일 뒤**다. 판이 마지막이면 이름·수치 **위**에 깔려서 진하게 칠할 수가
  없었고(0.22 가 상한이었다), 그래서 안 보였다. 맨 아래로 내리니 0.5 도 안전하다.
  `SetAsFirstSibling()` 은 **코드가** 한다 — 행 모체를 손대지 않아도 모든 행에 걸린다
  (`HpShield` · 아웃라인과 같은 방식).

⚠ 칸(`RectTransform`)은 **씬에** 있고 색·순서는 **코드에** 있다. 칸을 옮길 일이 생기면
  씬을 고칠 것 — 코드는 칸을 건드리지 않는다.

### UI-68-2. 로스터 행 — 얼굴을 <b>왼쪽 끝</b>으로, 죽은 자리 102px 를 없앴다

행은 382×104 이고 카드 그림(`Btn_Roster_Normal` 393×90 · 9-슬라이스 24/14/24/18)의
**안쪽 홈은 x 14~371 · y 18~90** 이다. 예전 배치는 그 홈을 **못 쓰고 있었다**.

| | 예전 | 이번 |
|---|---|---|
| 얼굴 액자 | x 26~94 (**왼쪽에 죽은 자리 26px**) | **x 12~80** — 홈에 붙음 |
| 이름 | x 102~208 | x 88~238 |
| 맡은 일 | x 214~278 | **x 272~368** — 오른쪽 끝 |
| 체력바 | x 102~276 (174) | **x 88~278 (190)** |
| 침식바 | x 102~276 (174) | **x 88~278 (190)** |
| 유물 3칸 | x 236~306 | **x 298~368** — 오른쪽 끝 |
| 안 쓰는 폭 | 왼 26 + 오른 76 = **102px** | **0** |

* 얼굴과 액자의 **6px 간격은 그대로**다(액자 68 · 얼굴 56 · 안쪽으로 6). 예전 관계를 옮기기만 했다.
* 「맡은 일」과 유물 띠의 **오른쪽 끝을 368 로 맞췄다** — 둘이 세로로 한 줄에 선다.
* 유물 띠는 `relicIconStepPixels: -24` 라 **왼쪽으로 자란다**(346 → 322 → 298).
  오른쪽 끝에 붙은 아이콘이 오른쪽으로 자라면 칸 밖으로 나간다 — 그 규칙은 UI-67 그대로다.
* 체력바·침식바의 자식(채움·잔상·%글자)은 전부 **stretch 앵커**라 폭만 늘리면 따라온다. 손댈 것이 없었다.

★ 배치는 **9-슬라이스를 실제로 굽고 그 위에 상자를 그려** 확인했다 — 카드 홈의 위치는
  «대충 24px» 이 아니라 그림에서 잰 값이다(속살 `#212B38` 이 시작되는 x 14 · y 18).

### UI-68-3. ★★ 언어 버튼 — <b>그 버튼만 `Image` 가 없었다</b>

**증상** — *"언어 변경 버튼이 없음 현재 언어만 나옴"*.

`SettingsPanel` 의 언어 토글은 **기능이 다 있었다**(`HandleToggleLanguage` · `PlayerPrefs` ·
`OnLanguageChanged` 재그리기). 씬에도 `Body/LanguageButton` 이 있었다. 그런데 —

```
QuitButton        [RectTransform, CanvasRenderer, Image, Button]
HotkeyButton      [RectTransform, CanvasRenderer, Image, Button]
LanguageButton    [RectTransform,                        Button]   ← 판이 없다
```

`Image` 가 없으니 **① 안 보이고 ② 눌리지도 않는다** — `Graphic` 이 없는 곳은
레이캐스트가 그냥 통과한다. 자식 `Label` 만 남아 **글자만 떠 있었다.**
그게 «현재 언어만 나온다» 의 정체다.

**고침** — `HudTheme.EnsureButtonSkin(Button, family)` 신설. 없으면 `Image` 를 붙이고,
계열의 네 그림(`Btn_Action_Normal/Hover/On/Off`)을 `Resources` 에서 읽어 꽂고,
`targetGraphic` 과 `SpriteState` 까지 맞춘다. 즉 **옆의 저장·로비·재시작·나가기와 똑같이** 만든다.

★ **그림은 코드가 꽂는다** — MCP 로는 씬 오브젝트에 `Sprite` 참조를 넣을 수 없다(8절 4번).
  실제로 이번에도 `update_component` 로 `m_Sprite` 를 넣어 봤고 **`null` 로 남았다**.
  칸·컴포넌트는 MCP 가, 그림은 코드가 — 로스터의 액자·부대 띠와 같은 분업이다.
★ `PaintButton` 과 **역할이 다르다**. `PaintButton` 은 «이미 붙은 그림» 에서 계열을 읽어
  상태만 갈아끼운다 — 그림이 아예 없으면 할 일이 없다. `EnsureButtonSkin` 은 그 앞 단계다.
⚠ **이미 그림이 있으면 손대지 않는다** — 씬에서 다른 계열을 골라 뒀을 수 있다.

### UI-68-4. ★★ 얼굴을 <b>카드 밖</b>으로 — 목록 왼쪽에 <b>자리를 판다</b>

유저: *"유물 칸 조금 더 크게 해주고 초상화를 아예 버튼 이미지 영역 밖에 자연스럽게 붙여서 더 크게 해줘"*

**막다른 길이 둘 있었다.**

1. **행 안에서 왼쪽으로 밀 수 없다.** 카드 그림(`Image`)은 행의 `RectTransform` 을 <b>꽉 채운다</b>.
   행 안이면 어디에 두든 «버튼 이미지 영역 안» 이다.
2. **행 밖으로 내밀면 잘린다.** 뷰포트에 `RectMask2D` 가 있어서 목록 왼쪽 끝(x 0)을 넘는 것은
   그대로 잘려 나간다. 행이 목록 폭을 꽉 채우고 있으니 내밀 데가 없다.

**그래서 목록에 «왼쪽 여백» 을 판다** — `List` 의 `VerticalLayoutGroup.padding.left = 64`.

```
     0        64                                          382   ← List(뷰포트) 폭
     ├─ 여백 ─┼──────────── 카드(행) 318 ────────────────────┤
     │  얼굴 96x96 (x 2~98)                                  │
             └ 34px 만 카드에 걸친다 = «붙어 있다»
```

`childControlWidth` 가 켜져 있어 행 폭이 <b>저절로</b> 382 → 318 로 준다. 얼굴은 행 기준
x −62 에 놓이므로 <b>목록 기준으로는 x 2</b> — 뷰포트 안이라 <b>안 잘린다</b>.
카드를 좁히려고 자식 `Card` 를 새로 만들 필요도, `Button`·`PaintButton` 배선을 바꿀 필요도 없었다.

★ **얼굴이 카드 «위» 에 그려진다** — 카드는 행 자신의 `Image`(가장 먼저 그려진다)이고
  얼굴·액자는 자식이다. 그래서 34px 겹친 부분이 카드의 왼쪽 테두리 장식을 덮어
  «판때기를 카드 끝에 물린» 모양이 된다. 그것이 «자연스럽게 붙여서» 의 뜻으로 읽었다.
⚠ **얼굴에 `raycastTarget` 을 켰다.** 카드 밖으로 나간 부분에는 카드의 판이 없어서
  꺼 두면 <b>그 자리만 안 눌린다</b> — «얼굴을 눌렀는데 선택이 안 된다» 가 된다.
  자식이 받은 클릭은 행의 `Button` 으로 올라간다.

| | 앞(UI-68-2) | 이번 |
|---|---|---|
| 목록 왼쪽 여백 | 0 | **64** |
| 행(카드) 폭 | 382 | **318** |
| 얼굴 액자 | 68 (행 안 x 12~80) | **96** (행 밖 x −62~34) |
| 얼굴 | 56 | **84** |
| 유물 칸 | 22 · 간격 24 | **28 · 간격 30** |
| 이름·체력·침식 | x 88~ | x 44~194 |
| 맡은 일 · 유물 띠 | 오른쪽 끝 368 | 오른쪽 끝 **304** |

* 유물 칸은 22 → **28**(+27%). 간격도 −24 → **−30** 으로 같이 넓혔다 — 칸만 키우면 서로 붙는다.
  띠는 여전히 <b>왼쪽으로 자란다</b>(오른쪽 끝에 붙어 있으므로 · UI-67 규칙 그대로).
* 액자 96 은 행 높이 104 안에 든다(위아래 4px). **행 높이는 안 건드렸다** — 키우면 한 화면에
  보이는 캐릭터 수가 준다.
* 부대 색 판은 앵커가 «네 귀 늘이기» 라 <b>손댈 것이 없었다</b> — 행이 좁아지면 같이 좁아진다.

### UI-68-5. 로비 환경 설정 — <b>«사람에 딸린 값» 만</b> 옮겼다

유저: *"로비화면 환경 설정에도 환경 설정 버튼들 같이 붙여주고"*

게임 중 환경 설정의 버튼은 <b>두 갈래</b>다. 통째로 옮기면 로비에 «저장하기» 가 생긴다.

```
판에 딸린 것   : 저장 · 로비로 · 다시 시작 · 저장 않고 나가기   → 로비에는 없다(저장할 판이 없다)
사람에 딸린 것 : 음량 · 언어 · 단축키 · 도움말 기억             → 로비에도 있어야 한다
```

뒤쪽 넷은 전부 `PlayerPrefs` 에 남는 값이라 <b>어느 씬에서 만져도 같다</b>. 로비 창에는
음량뿐이었으므로 **언어 · 단축키 · 도움말 다시 보기** 셋을 더했다(창 520×260 → **520×400**).

**★ 규칙이 두 벌이 되는 것을 먼저 막았다** — `UI/LanguageSetting.cs` 신설.
언어의 «열쇠(`ls_language`) · 기본값(한국어) · 이름(한국어/English)» 이 두 창에 각각 있으면
한쪽만 고쳤을 때 «로비에서 바꿨는데 게임에서 안 바뀐다» 가 된다. `SettingsPanel` 의
`languagePrefKey` 필드를 **없애고** 그쪽을 정본으로 삼았다(음량이 `VolumeSlider` 하나에
모여 있는 것과 같은 자리).

**단축키 창은 로비에 «빈 루트» 하나면 된다.** `HotkeyPanel` 은 줄 수가 기능 수를 따라가서
<b>안쪽을 스스로 짓는다</b>(UI-59 의 판단). 그래서 `UI_Root/HUD_Hotkeys` 오브젝트 하나만
만들면 로비에서도 그대로 열린다. `HotkeyService` 는 `static` 이고 `HudLog`·`HudExclusive` 도
정적이라 <b>게임 씬이 아니어도 안전하다</b>.
⚠ **루트는 켠 채로 저장한다** — 끄면 `Awake` 가 안 돌아 `Instance` 가 영영 null 이다
  (그 클래스가 이미 겪고 적어 둔 함정).

**도움말만 서비스가 없다.** `HelpService` 는 게임 씬에만 있다. 로비에서는 그 서비스가
결국 지우는 것과 <b>같은 열쇠</b>(`HelpService.PrefsKey`, `public const`)를 직접 지운다.

**만드는 일은 MCP 가, 참조는 에디터 메뉴가.** 스프라이트·폰트는 MCP 로 못 넣으므로
스크립트가 마지막에 세 메뉴를 스스로 부른다 — `LastSanctuary/UI/배선` ·
`LastSanctuary/UI/글자 여백` · `LastSanctuary/폰트/네오 둥근모 …`.
그 결과 새 버튼 셋이 <b>게임 쪽과 같은 그림</b>을 받았다(실측):

```
LanguageButton   온폭 480x46  → Btn_Action_*   (저장·로비·재시작·나가기와 같은 계열)
HotkeyButton     반폭 238x46  → Btn_Panel_*    (게임 쪽 단축키·도움말 짝과 같은 계열)
HelpResetButton  반폭 238x46  → Btn_Panel_*
```

⚠ 스크립트는 씬을 갈아 끼우므로 <b>Proto_01 을 먼저 저장</b>하고, 끝나면 <b>Proto_01 로
  되돌려 놓는다</b> — 에디터를 켜 둔 사람의 자리를 바꾸지 않는다.

### 검산

* `recompile_scripts` — 오류 0 · 경고 0. 콘솔 오류 0.
* 씬 저장 후 YAML 재확인 — 행 여덟 칸 좌표 · `SquadTint` 칸(pos 1.5,-2 · size -25,-32) ·
  `squadTintAlpha: 0.5` · 목록 `padding.left: 64` · `relicIconStepPixels: -30`.
* 로비 — 새 버튼 셋이 그림을 실제로 받았는지 YAML 로 확인:
  `LanguageButton` `752dc795…`(Btn_Action) · `HotkeyButton`·`HelpResetButton` `07895eeb…`(Btn_Panel).
  네 라벨 전부 네오 둥근모(`c9323a04…`).
* 행 배치는 **9-슬라이스를 실제로 구워** 상자를 겹쳐 눈으로 확인했다(카드 홈 x 14~307 · y 18~90).

### 아직 확인 못 한 것 (유저가 볼 것)

1. ★★ **부대 색 진하기 0.5** — 여섯 색이 갈리는지 눈으로 볼 것. 약하면
   `HUD_Roster > CharacterRosterPanel > squadTintAlpha` 만 올리면 된다(이제 0~1 전부 열려 있다).
   판이 글자 **아래**라 진해져도 이름·수치는 안 물든다.
2. ★ **얼굴이 목록 왼쪽 끝에 딱 붙는다**(List x 2) — 뷰포트에 2px 만 남는다.
   더 키우고 싶으면 `List` 의 `padding.left` 와 액자 x 를 **같이** 늘릴 것 — 한쪽만 늘리면 잘린다.
3. ★ **행 폭이 382 → 318 로 줄었다.** 「맡은 일」 칸이 86px 다 — 긴 상태 이름이 들어오면
   `HudTheme.FitText` 가 글자를 줄인다. 너무 작아 보이면 유물 띠를 한 칸 줄이거나 칸을 넓힐 것.
4. ★ **로비의 「단축키 설정」** — 그 창은 게임 기능 이름(캐릭터 생성·부대 설정…)을 그대로 보여준다.
   로비에서 봐도 이상하지 않은지 볼 것. 이상하면 로비에서는 그 버튼만 끄면 된다.
5. ⚠ **로비 환경 설정 창에는 판 그림(`Win_Frame`)이 없다** — 이번에 생긴 것이 아니라 원래 그랬다
   (`LastSanctuary/UI/배선` 의 창 목록에 `SettingsWindow` 가 없다). 게임 쪽 창과 톤이 다르다.
6. ⚠ **이번 작업과 무관**: `[Minimap] HUD_Minimap/DigMarks 를 찾지 못했습니다` 경고 — UI-69 에서 다뤘다.

### 씬 변경 여부

**있음.**
* `Proto_01` — `HUD_Roster/RowTemplate` 여덟 칸 재배치(2회에 걸쳐) · `HUD_Roster`
  (`squadTintAlpha` 0.22→0.5 · `relicIconStepPixels` -24→-30) · `List`(`padding.left` 0→64) ·
  `HUD_Settings/Body/LanguageButton`(+`Image`·`CanvasRenderer`).
* `Lobby` — `SettingsWindow` 520×260→520×400 · `Body` 에 `LanguageButton`·`HotkeyButton`·
  `HelpResetButton`·`Status` 신설 · `Volume` 이동 · `UI_Root/HUD_Hotkeys` 신설.

`save_scene` **3회**.

### 씬반영요청 목록

- 없음 (전부 MCP + 에디터 메뉴로 처리했다)

---

## UI-69. ★★★ 중립 사냥값이 <b>한 종에 다 실려</b> 있었다 · 엔딩 명단을 <b>다 적고 읽히게</b> · ★★ 미니맵 느낌표가 <b>왼아래 귀퉁이 기준</b>으로 찍히고 있었다 (2026-08-26, 26차)
...
| # | 지시 | 결과 |
|---:|---|---|
| 1 | *"엔딩 스토리 나올 때 남은 인물들 확실하게 다 나오게 하고 (그외 4명) 이런 식 말고"* | ✅ 접기를 없애고 **자동 크기**로 받는다 (UI-69-1) |
| 2 | *"글씨 안 보이니까 폰트 색 변경해서 가시성 높이고 왼쪽 정렬해줘"* | ✅ ★★ 배경이 **밝은 크림색**인데 글자도 밝은 회백색이었다 (UI-69-1) |
| 3 | *"종양 두더지 사냥 보상이 과도하게 높은데 전 구간에 나눠서 … 중립 몬스터 사냥값"* | ✅ ★★★ 마리당 795→340 · 한 판 수입의 58%→44% (UI-69-2) |
| 4 | *"미니맵에서 자꾸 느낌표들이 미니맵 바깥으로 나가는 버그"* | ✅ ★★ **앵커가 (0,0) 인데 «가운데 기준» 좌표를 넣고 있었다** (UI-69-3) |

**고친 C#** — `UI/EndingDirector.cs` · `UI/MinimapPanel.cs`
**새 스크립트** — [Tools/table_update_20260826_neutral_spread.py](Tools/table_update_20260826_neutral_spread.py)
**표 변경** — 「임시용 중립 몬스터」 4종 · 「능력치 및 공식 정리 · 웨이브 부하」 9열 × 30행 · 「계수」 5행
**씬 변경** — 있음(`Ending` 명단 칸) · `save_scene` **1회**

---

### UI-69-1. 엔딩 명단 — <b>다 적고</b>, <b>먹색</b>으로, <b>왼쪽</b>에

#### ★★ 「그외 N명」 — 접는 것 자체가 틀렸다

`rollMaxNames`(8)까지만 적고 나머지를 «… 그리고 N명» 으로 접고 있었다. **칸을 넘겨
화면 밖으로 흐르는 것**을 막으려던 것인데, 이 명단은 <b>그 판을 함께 치른 사람이 전부</b>다 —
아홉 번째 사람이 «그외 1명» 이 되면 그 판의 기록이 아니게 된다.

★ 흐르는 문제는 **접는 대신 «작게»** 로 받는다. TMP 자동 크기(`enableAutoSizing`)가
칸에 맞춰 26 → 최소 14 까지 줄인다. `HudTheme.FitText` 가 유물 설명에서 «자르지 않고
줄인다» 를 택한 것과 <b>같은 판단</b>이다 — 자르면 정보가 사라지고, 줄이면 안 사라진다.

실측: 전사 6 + 생존 12 = **18명(21줄)** 이면 26 → **21px** 로 앉는다. 칸도 900×460 →
**860×620** 으로 키우고 `rollRaise` 90 → **140** 으로 올렸다(자막 위로 비켜난다).

#### ★★★ 「글씨가 안 보인다」 — <b>밝은 그림 위에 밝은 글자</b>

명단은 **컷 2** 에서만 뜬다(`showRoll: 1` 이 붙은 조각이 `ending_caption_02_4` 하나뿐).
그 배경 `Ending/BG_02` 는 성문 앞 도열 장면인데 <b>하늘도 석조도 전부 밝은 크림색</b>이고,
명단이 놓이는 화면 **가운데**가 그중에서도 가장 밝다(아치 안쪽의 빛).

그런데 글자는 HUD 기본색 `HudTheme.TextMain`(0.88, 0.92, 0.94) — <b>밝은 회백색</b>이었다.
머리글은 더 심해서 `<color=#8E9AA6>` 회청색이었다. 밝은 것 위에 밝은 것을 얹었으니
안 보이는 것이 당연했다.

```
   본문   0.88,0.92,0.94 (밝은 회백)  →  0.11,0.09,0.07 (먹색)
   머리글 #8E9AA6 (회청)             →  #6B4A21 (짙은 갈색)
   테두리 없음                        →  1,0.97,0.90 알파 0.95 · 2px  (크림색 후광)
   정렬   Center                      →  Left        ← 유저 지시
```

★ **«밝은 종이에 쓴 먹»** 으로 뒤집었다 — 이 장면의 톤과도 맞는다. 테두리가 있어
어두운 깃발 쪽으로 밀려도 읽힌다. **네 값 전부 인스펙터로 뺐다** — 배경 그림이 바뀌면
다시 뒤집어야 하는 값이기 때문이다.

⚠ **세로는 «가운데»(`TextAlignmentOptions.Left`) 다.** `TopLeft` 로 두면 살아남은 사람이
셋뿐인 판에서 명단이 <b>칸 꼭대기에</b> 붙어 화면 위쪽에 외따로 뜬다. 가운데면 인원수와
무관하게 자리가 같다.

⚠ **테두리는 `Outline` 컴포넌트로 두른다.** TMP 의 `outlineWidth` 는 <b>공유 머티리얼</b>을
건드려서 같은 폰트를 쓰는 <b>화면의 모든 글자</b>(자막·건너뛰기 버튼)가 같이 두꺼워진다.

### UI-69-2. ★★★ 중립 사냥값 — <b>거리로만 곡선을 만들려다 한 종이 다 짊어졌다</b>

#### 무엇이 잘못돼 있었나

UI-67 은 «먼 종의 보상만 올리면 후반에만 수입이 오른다» 는 생각으로 1003(종양 두더지)과
에픽 넷을 **×3.0** 했다. 곡선의 **모양**은 맞았는데 **한 종이 그 곡선을 통째로 짊어졌다**.
...
두더지는 **체력이 14** 다 — 1002(8)·1004(15)와 같은 급이다. «멀다» 는 것 말고는 비싼
이유가 없는데 마리당 값이 중반 종의 **17배** 였다. 그래서 26~30 구간이 **한 판 중립
수입의 58%** 를 차지했고, 그 구간은 사실상 «두더지 밭에 원정 보내는 것» 하나로 굴러갔다.

#### ★ 먼저 「구간별 사냥 구성」 모델을 만들었다

시트의 «중립 수입/웨이브» 는 <b>가정치</b>라 종별 값에서 유도되지 않는다 — 종별 값을
바꿔도 총 수입이 얼마가 되는지 알 수가 없었다. 그래서 <b>옛 수입에서 구성을 되짚었다</b>.

| 구간 | 1001 | 1002+1004 | 1003 | 에픽 | 모델 | 시트 | 오차 |
|---:|---:|---:|---:|---:|---:|---:|---:|
| 1~5 | 20 | 0 | 0 | 0.0 | 180 | 176 | +2.3% |
| 6~10 | 39 | 0 | 0 | 0.0 | 351 | 352 | −0.3% |
| 11~15 | 20 | 18 | 0 | 0.0 | 999 | 1013 | −1.4% |
| 16~20 | 20 | 46 | 0 | 0.0 | 2273 | 2291 | −0.8% |
| 21~25 | 20 | 40 | 3 | 0.5 | 5510 | 5282 | +4.3% |
| 26~30 | 20 | 40 | 10 | 1.2 | 12650 | 12588 | +0.5% |

총합 109,815 vs 108,510 (**+1.2%**). 구간별로도 ±4.3% 안이다 —
**그래서 이 모델로 새 값을 고를 수 있다.** 모델은 스크립트 안에 남는다(또 잃지 않는다).

#### 새 사냥값 — 총 수입은 그대로, <b>실리는 자리만</b> 옮긴다

| 종 | 옛 | 새 | 마리당 평균 | 지속 수급 |
|---|---|---|---|---|
| 1001 종양 거미 | 6~12 | **11~17** | 9 → 14 | 43 → 67 E/s |
| 1002 종양귀 | 34~57 | **62~98** | 46 → 80 | 116 → 203 E/s |
| 1004 고르도네 | 34~57 | **62~98** | 46 → 80 | 116 → 203 E/s |
| 1003 종양 두더지 | 600~990 | **260~420** | 795 → 340 | **604 → 258 E/s** |
| 1101~1104 에픽 | 그대로 | 그대로 | — | — |

★ **지속 수급이 단조롭게 오른다**(67 → 203 → 258). 마리당 값도 «멀리 갈수록 비싸다» 가
  유지된다(14 → 80 → 340) — 200~320 타일 원정을 갈 이유는 남는다.
★ **에픽은 안 건드렸다** — 유저가 문제 삼지 않았고, 20분 리스폰·레벨 제한(10/15/20/25)이
  이미 «한 판에 한두 번의 큰 상» 으로 잠가 둔다.
...
총 중립 수입 108,510 → 108,555 (+0.04%)  →  착지점 «w30 12명 Lv35» 그대로 (Lv35.2)
```

#### ⚠⚠ 대가 — <b>중반이 세진다</b>

후반에 오던 돈이 중반으로 오므로 파티 레벨이 오른다. 시트의 부하 열을 다시 계산했다.

| 웨이브 | 평균 Lv (전) | 잡몹 부하 (전) |
|---:|---|---|
| 10 | 11.2 (9.4) | 0.21 (0.25) |
| 15 | 15.0 (11.9) | 0.23 (0.29) |
| 20 | **25.9 (20.4)** | 0.27 (0.36) |
| 25 | 28.5 (24.4) | 0.31 (0.38) |
| 30 | 35.2 (35.0) | 0.41 (0.41) |

**16~25 웨이브가 그만큼 싱거워진다.** 이것은 «전 구간에 나눠서» 의 <b>필연</b>이다 —
총 수입을 그대로 두고 두더지를 57% 깎으면 그 돈은 반드시 앞 구간으로 간다.
몬스터 수치는 **여기서 손대지 않았다** — UI-67 의 유저 확정 ②(«몬스터는 그대로»)를
뒤집는 일이라 따로 물어야 한다.

### UI-69-3. ★★ 미니맵 느낌표 — <b>«가운데 기준» 좌표를 «왼아래 귀퉁이» 에 넣고 있었다</b>

`DrawDigMarks` 는 표식 자리를 이렇게 잡는다:

```csharp
Rect area = _digMarkParent.rect;                       // 피벗(가운데) 기준 → −140 … +140
item.anchoredPosition = new Vector2(
    Mathf.Lerp(area.xMin, area.xMax, u), ...);
```

`RectTransform.rect` 는 **피벗 기준**이라 그 값의 범위가 `−140 … +140` 이다.
그런데 씬의 모체 `DigMarks/MarkTemplate` 은 앵커가 **(0,0)** — <b>부모의 왼아래
귀퉁이 기준</b>이었다. 그래서

```
  u = 0.0  →  x = −140  →  미니맵 왼쪽 «바깥» 140px      ← 새는 자리
  u = 0.5  →  x =    0  →  미니맵 왼쪽 «끝»
  u = 1.0  →  x = +140  →  미니맵 «한가운데»
```

즉 <b>지도 왼쪽 절반(u &lt; 0.5)·아래쪽 절반(v &lt; 0.5)의 표식이 전부 판 밖으로 샜다.</b>
발굴 자리를 «맵 전체» 로 넓힌 UI-64 이후로 그 절반이 늘 있었다.

**고침 셋**

1. **복제본의 앵커·피벗을 코드가 (0.5, 0.5) 로 박는다.** 모체를 인스펙터에서 만지다
   앵커가 다시 틀어져도 안 샌다 — «자리 규칙은 한 곳» 이다.
2. **귀퉁이에서 반지름만큼 안으로 물린다.** 자리는 «칸의 가운데» 인데 그림은 14px 이라,
   지도 끝의 칸은 7px 이 판 밖으로 나간다. 튀는 높이(`bounce`)도 위쪽 끝에서 같은 짓을 한다.
   → 자르지 않고 **끝에 붙여** 둔다. «지도 끝에 뭔가 있다» 를 그대로 알려 주는 편이 낫다.
3. ⚠ **지도 밖 칸에서 `slot++` 를 하던 것을 뺐다.** 그 칸은 자리를 못 받고 넘어가는데
   `HideDigMarksFrom` 은 `slot` <b>뒤만</b> 끄므로 **지난 프레임의 자리에 그대로 켜져
   남았다** — «안 지워지는 느낌표» 의 정체다. 칸을 안 쓰고 넘어가면 다음 표식이 그 칸을 이어 쓴다.

### 검산

* `recompile_scripts` — 오류 0 · 경고 0. 콘솔 오류 0.
* 경제 — 구성 모델이 옛 시트를 재현하는지 **먼저** 확인하고(구간 ±4.3% · 총합 +1.2%)
  그 모델로 새 값을 골랐다. 총 수입 +0.04% · w30 Lv35.2.
* 표 → 에셋 반영 — `sync_tables_to_assets.py` 뒤 에셋 재확인:
  1001 `11~17` · 1002 `62~98` · 1003 `260~420` · 1004 `62~98` · 에픽 넷 그대로.
* 엔딩 — `BG_02` 위에 새 색·정렬·자동 크기로 **실제로 그려** 확인했다(18명 21줄 → 21px).

### 아직 확인 못 한 것 (유저가 볼 것)

1. ⚠⚠ **16~25 웨이브가 싱거워진다** (UI-69-2 의 표). 부하 0.36 → 0.27 (w20).
   너무 쉬우면 **그 구간 몬스터를 잡을지** 를 정해 줄 것 — 몬스터 수치는 유저 확정 사항이라
   임의로 안 건드렸다.
2. ★ **사냥 구성 모델은 «되짚은» 것이다.** 실제로 한 웨이브에 종양귀를 46마리 잡을 수
   있는지는 플레이로 봐야 한다. 못 잡으면 중반 수입이 모델보다 적게 들어온다.
3. ★ **엔딩 명단 21px** — 18명이면 이 크기다. 화면에서 작아 보이면 `rollSize` 를 키우거나
   `rollFontSizeMin` 을 올릴 것(올리면 넘칠 수 있다).
4. ★ **엔딩 머리글 `#6B4A21`** — 밝은 배경에서 본문(먹색)보다 흐리다. 안 보이면 더 짙게.
5. ★ **미니맵 표식이 이제 «끝에 붙는다»** — 지도 가장자리의 발굴 자리는 판 테두리에
   닿아 보인다. 잘라 없애는 편이 나으면 `DigMarks` 에 `RectMask2D` 를 붙이면 된다.

### 씬 변경 여부

**있음.** `Ending/EndingDirector` — `rollSize` 900×460 → 860×620 · `rollRaise` 90 → 140.
(새 칸 여섯은 C# 기본값으로 태어난다.) `save_scene` **1회**.
...
## UI-70. ★★ <b>X 버튼이 창마다 다른 자리에</b> 있었다 · 성장 창 유물 칸이 머리글을 덮고 있었다 · ★★ 발굴 <b>첫 창만</b> 한글이던 이유 · 로비 타이틀 교체 (2026-08-27, 27차)

### 무엇을 / 왜

유저 지시 여섯. **2번과 5번은 같은 글자를 두고 방향이 뒤집혔다** — 아래에 그대로 적는다.

| # | 지시 | 결과 |
|---:|---|---|
| 1 | *"부대설정, 전술지침, 캐릭터성장, 토벌지시, 유물관리, 환경설정, 도움말 … x버튼의 위치가 어긋나있는 경우가 있어"* | ✅ ★★ 일곱 창이 <b>자리 다섯 가지 · 크기 네 가지</b>였다 (UI-70-1) |
| 2 | *"캐릭터 성장 창에 유물 장착칸이 너무 위에 있어서 «장착할 유물» 텍스트를 가리고 있어"* | ✅ 칸이 머리글 <b>아래에서</b> 시작하게 했다 → <b>5번으로 뒤집혔다</b> (UI-70-2) |
| 3 | *"우측 하단에 있는 «닫기» 버튼은 더 이상 필요 없으니까 삭제해줘"* | ✅ 씬 오브젝트와 지도 칸을 같이 지운다 (UI-70-2) |
| 4 | *"유물 발굴 시 처음에 한 해서 언어를 영어로 한 상태에서도 한글로된 문구가 나와"* | ✅ ★★ <b>제목을 <c>Awake</c> 보다 먼저 읽고</b> 있었다 (UI-70-3) |
| 5 | *"«Equipped Relics» 라고 나오는 투명한 회색 글씨 지워줘 … 이름 글씨 뒤에 있는 회색글씨"* | ✅ 머리글을 <b>지웠다</b> — 2번의 이유가 사라져 띠를 되돌린다 (UI-70-2) |
| 6 | *"로비에 쓸 타이틀이 바뀐거야 … 볼트파일 다시 확인해서 진행해"* | ✅ ★★ 볼트 원본이 <b>그날 15:05</b> 에 바뀌어 있었다 (UI-70-4) |

**고친 C#** — `UI/RelicCardStrip.cs` · `UI/CharacterGrowthPanel.cs` · `UI/RelicDigPanel.cs` · `UI/UiLocalizer.cs`
**돌린 스크립트** — [Tools/import_lobby_art.py](Tools/import_lobby_art.py) (고치지 않고 그대로 돌렸다)
**에셋 변경** — `Resources/UI/Lobby/LobbyTitle.png`
**씬 변경** — 있음 (`Proto_01` · `Lobby` 둘 다)

⚠ **이 절은 한 번 통째로 디스카드됐다가 다시 넣은 것이다.** 머지 문제로 작업본이 날아가
`19660e09`(성별 추가) 위에서 다시 올렸다. C# 넷 중 `CharacterGrowthPanel` · `UiLocalizer` 는
그 커밋도 건드린 파일이라 <b>옛 패치를 그대로 덮지 않고</b> 새 내용에서 자리를 다시 찾아 넣었다.

---

### UI-70-1. ★★ X 버튼 — <b>틀이 같은데 자리만 달랐다</b>

일곱 창은 <b>틀 그림이 같다</b> — 전부 `UI/Frames/Win_Frame`(9슬라이스 · 테두리 좌23 · 우23 ·
상21 · 하22)을 쓴다. 그러면 X 도 틀의 오른윗 귀퉁이에서 <b>같은 거리</b>에 있어야 하는데,
재어 보니 그렇지 않았다.

```
   창            부모      피벗    자리        크기     오른쪽 여백 / 윗 여백
   부대설정      Header   (1,1)   (-24,-22)   44×44      24 / 22
   전술지침      Header   (1,1)   (-24,-20)   44×44      24 / 20
   캐릭터성장    Header   (1,1)   (-24,-20)   44×44      24 / 20
   토벌지시      창       (1,1)   (-27,-27)   40×40      27 / 27
   유물관리      창       (.5,.5) (-43,-43)   32×32      27 / 27
   도움말        창       (.5,.5) (-46,-46)   32×32      30 / 30
   환경설정      Header   (1,.5)  (-12,  0)   34×30      12 /  9   ← 눈에 띄는 것
```

★ <b>환경설정이 범인이다.</b> 오른쪽 12 · 윗 9 인데 <b>틀 테두리가 우23 · 상21</b> 이다 —
  즉 버튼이 틀 안이 아니라 <b>테두리 그림 위에</b> 올라타 있었다. 다른 창을 보다
  환경설정을 열면 X 가 불쑥 튀어나온 것처럼 보인다.
★ **크기도 네 가지였다.** `Btn_Close_*` 원화는 <b>32×32 · Point 필터</b>다. 44 는 1.375배 ·
  40 은 1.25배 — 픽셀을 <b>정수배가 아니게</b> 늘려 테두리가 고르지 않게 번진다.
  환경설정의 34×30 은 한술 더 나쁘다 — <b>가로세로 비율이 깨져</b> 원이 타원이 된다.

**고침** — 일곱을 한 줄로 맞췄다. 자리 규칙은 <b>한 벌</b>이다.

```
   anchor (1,1) · pivot (1,1) · anchoredPosition (-27, -27) · sizeDelta 32×32
```

★ **27 을 고른 이유** — 틀 테두리(우23 · 상21) <b>안쪽</b>이면서, 이미 서로 맞아 있던
  두 창(토벌지시 · 유물관리)과 <b>발굴 창</b>(`HUD_Dig`)의 값이다. 이미 맞는 쌍을
  기준으로 잡으면 움직이는 창이 적다.
★ **부모를 그대로 둔 이유** — `Header` 가 붙은 네 창은 전부 <b>Header 의 오른윗 모서리 =
  창의 오른윗 모서리</b> 다(폭이 창과 같거나 가로로 늘어나 있다). 값 하나로 같은 자리가
  되므로 옮기지 않았다 — 옮기면 <b>배선이 끊길 위험</b>만 생긴다.
★ **X 글자는 그림이 아니라 <c>Label</c> 이다.** `Btn_Close_Normal` 은 <b>빈 판때기</b>고
  「X」는 그 안의 TMP 다(일곱 다 Center/Middle · 같은 폰트라 이쪽은 문제가 없었다).

⚠ 환경설정은 창이 520 으로 좁아 제목 칸(18~464)과 버튼 칸(461~493)이 <b>3px 겹친다</b>.
  제목은 <b>가운데 정렬</b>이라 글자가 그 끝까지 가지 않는다 — 유물관리도 예전부터 같은
  3px 겹침이었고 문제가 없었다. 영어 제목이 길어져 닿으면 `Header/Title` 을 왼쪽으로 민다.

### UI-70-2. 성장 창 — 머리글이 <b>칸 뒤에 깔려</b> 있었다 → <b>머리글을 지웠다</b>

#### 무엇이 잘못돼 있었나

`RelicCardStrip` 은 칸 셋을 띠(`RelicBar`) 안에 <b>y=0 · 띠 높이 그대로</b> 깐다.
그런데 머리글 `RelicBar/Head`(「장착한 유물」 / 「Equipped Relics」)도 <b>같은 띠 안</b>에
있고(y −18 · 높이 16), 칸은 띠에 <b>나중에</b> 붙는다 — 그리는 순서상 판때기가 머리글
위로 올라간다. 칸이 «너무 위에 있는» 것이 아니라 <b>머리글 자리를 안 비우고 있었다</b>.

머리글이 <b>완전히</b> 가려지지도 않았다. 유물 이름(칸 안 `Name`, y −14~−38)과 머리글
(y −10~−26)이 <b>16px 겹쳐서</b>, 이름 글자 뒤로 회색 글씨가 비쳐 보였다 —
유저가 붙인 그림의 «The Cooling Fever-Break 글씨 뒤에 있는 회색글씨» 가 이것이다.

#### ★ 처음엔 «자리를 비웠고», 유저 지시로 <b>머리글을 지웠다</b>

2번 지시(«가리지 않도록 아래로 내려주고»)대로 먼저 띠를 122 로 키우고 칸을 32 내렸다.
그 다음 5번 지시(«그 회색 글씨 지워줘»)가 와서 <b>머리글 자체를 지웠다</b>.

★ **지운 뒤에는 띠를 되돌렸다** (122 → **90** · `ChangeButton` y −77 → **−42**).
  칸을 내리라고 한 <b>이유가 머리글이었는데</b> 그 머리글이 없어졌으므로, 내린 채로 두면
  띠 위쪽에 <b>아무것도 없는 32px 판때기</b>만 남는다. 되돌리면 칸 속(이름 · 효과 두 줄)이
  예전 그대로다.
  ⚠ 그래도 <b>칸을 아래로 내리고 싶다</b>면 씬에서 `RelicBar` 높이만 키우면 된다 —
    아래 `topReserved` 가 그대로 살아 있다.

#### 코드는 <b>남겼다</b> — `topReserved`

`RelicCardStrip.Build` 에 <b>위에 비워 둘 높이</b>를 넣는 길(`topReserved`)은 남아 있다.
성장 창이 `RelicBar/Head` 칸에서 «아래끝까지» 를 재어 넘기고, <b>머리글이 없으면 0</b> 이다.

★ **숫자를 안 박은 이유** — 머리글이 어디 있는지는 <b>씬이 정한다</b>. 지금은 머리글이
  없어 0 이지만, 나중에 다시 얹으면 <b>코드를 안 고쳐도</b> 칸이 알아서 내려간다.
  자리 규칙을 두 곳에 두지 않는 것은 UI-69-3 과 같은 판단이다.
⚠ `Build` 는 `topReserved` 를 <b>칸에 60px 는 남는 선에서만</b> 받는다. 옛 폴백
  경로(`Stats/RelicSlot`, 58px)로 되돌린 씬에서 칸이 «선» 이 되는 것을 막는다.

#### 지운 것 둘 — <b>지도에서도 뺀다</b>

```
   HUD_Growth/RelicBar/Head          삭제  ← 5번 지시
   HUD_Growth/Footer/CloseButton     삭제  ← 3번 지시 (Label 까지 둘)
```

`CharacterGrowthPanel.HookClose("Footer/CloseButton")` 도 뺐다(닫는 길은 머리글의 X 하나다).
⚠ **`UiLocalizer` 지도에서 두 칸을 같이 빼는 것이 중요하다** — 지도는 «창은 있는데
  그 안 경로가 없다» 를 <b>경고로 알린다</b>(그것이 그 검산의 값이다). 안 뺐으면
  부팅마다 가짜 경고가 두 줄 찍혔다.
★ `Footer` 자체와 스트링 키 `ui_growth_relic_head` 는 <b>남겨 둔다</b> — `Footer` 는 그림이
  없는 빈 칸이라 지우면 다른 창과 계층이 갈라지고, 스트링 표는 <b>엑셀이 정본</b>이라
  코드 쪽에서 행을 지울 자리가 아니다(안 쓰는 키는 해롭지 않다).

### UI-70-3. ★★ 발굴 창 — <b>제목을 <c>Awake</c> 보다 먼저 읽고</b> 있었다

«처음에 한 해서» 가 그대로 단서다. 발굴 창은 씬에 <b>비활성</b>으로 저장돼 있어
`Awake` 가 «처음 켜질 때» 돌고, 그 안에서 `LocalizeLabels()` 가 문구를 받아 온다.
그런데 <b>그 SetActive 가 `Show()` 안에서 일어난다</b>.

```csharp
public void PresentSite(...)
{
    Bind();                              // ← 아직 비활성 · Awake 는 안 돌았다
    ...
    Show(titleDiscover, body, null);     // ← 제목을 «여기서» 읽는다  ←← 범인
}
void Show(string title, ...)
{
    gameObject.SetActive(true);          // ← Awake 가 «이제» 돌며 영어로 바꿔놓는다
    _title.text = title;                 // ← 하지만 이미 넘어온 한글을 쓴다
}
```

즉 <b>첫 창의 제목만</b> 인스펙터에 구워진 한글이고, 둘째부터는 영어다.
본문·선택지는 멀쩡했다 — 본문은 표(`relic_dialogue_*` 41줄 · 영어가 다 채워져 있다)에서
읽고, 선택지는 `Show()` <b>뒤에</b> 붙어 그때는 이미 Awake 가 돈 다음이었다.

**고침** — `LocalizeLabels()` 를 `Bind()` 맨 앞으로 옮겼다. 모든 입구(`PresentSite` ·
`PresentResult` · `PresentBossDrop`)가 `Bind()` 를 먼저 부르므로 <b>첫 창부터</b> 맞는다.
⚠ `_bound` 가드 <b>위</b>다 — 아래에 두면 두 번째부터 안 돌아 언어를 바꿔도 안 따라온다.
★ 덤으로 <b>보스 드랍이 첫 창인 상황</b>도 같이 나았다 — `PresentBossDrop` 도 제목을
  같은 순서로 읽었으므로 «빼앗은 것» 이 한 번 한글로 떴다.

### UI-70-4. 로비 타이틀 — <b>볼트 원본</b>이 바뀌어 있었다

#### ★★ 왜 «안 바뀌었다» 고 잘못 답했나

처음엔 <b>프로젝트 안</b>만 봤다. `Resources/UI/Lobby/LobbyTitle.png` 는 HEAD 와
<b>해시가 같고</b>(f0e4890…) mtime 도 8/25 11:14 이라 «바뀐 적 없다» 로 읽혔다.

★ 그게 아니었다. **원본은 볼트에 있다** — `Tools/import_lobby_art.py` 가
`볼트/리소스/Loby/Title.png` 를 읽어 <b>검은 배경을 알파로 따서</b> Resources 로 굽는다.
볼트의 `Title.png` 가 **2026-08-27 15:05** 에 바뀌어 있었고(1536×1024 → **1686×933**),
<b>임포트를 안 돌려서</b> 프로젝트 쪽이 옛 그림 그대로였던 것이다.

⚠ **다음에는 볼트부터 본다.** 그림·표는 정본이 볼트고 프로젝트 파일은 <b>구운 결과</b>다 —
  프로젝트만 보고 «안 바뀌었다» 고 답하면 이 착각이 되풀이된다. `git status` 가 깨끗한 것은
  «원본이 그대로다» 가 아니라 <b>«아직 안 구웠다»</b> 는 뜻일 수 있다.

#### 무엇을 했나

`import_lobby_art.py` 를 <b>고치지 않고 그대로</b> 돌렸다 — 그림이 바뀌어도 스크립트는
그대로 도는 것이 원래 설계다(그 파일의 «멱등»).

```
   타이틀 원본  1686×933 RGB → 검은 배경 알파 처리
   LobbyTitle   1280×751  →  1280×746
   씬 Title 칸    757×444  →  762×444   (y −7 그대로)
   씬 Vignette   1135×731  → 1143×731   (y −229 그대로)
   titleRisePixels          311          ← 이미 311 이라 안 건드렸다
```

★ **칸 크기는 손으로 안 맞춘다** — 로고에 구운 그림자·글로우 여백이 그림의 10% 가까이라
  «칸 = 보이는 크기» 가 아니다. 스크립트가 계산해 찍어 준 값을 <b>그대로</b> MCP 로 넣었다
  (그 파일의 ★ · 100-9절과 같은 방식). 손으로 맞추면 배경 인물의 얼굴을 가린다.
★ **`LobbyBg.png` 는 되돌렸다** — 스크립트가 넷을 다 굽는데 배경 원본(`BG.png`)은
  8/26 그대로다. 다시 구우면 그림은 같고 <b>바이트만</b> 달라져(2.6MB) 잡음이 된다.
  `.meta` 넷도 줄바꿈만 LF 로 바뀌어서 CRLF 로 되돌렸다 — <b>GUID 는 안 바뀌었다</b>(확인함).

⚠ 로비 씬을 고치려면 <b>씬을 갈아타야</b> 한다. `Proto_01` 이 <c>Is Dirty: false</c> 인 것을
  확인하고 `Lobby` 로 옮겨 고치고 저장한 뒤 `Proto_01` 로 되돌린다.

### 검산

* `recompile_scripts` — 오류 0 · 경고 0. 콘솔 오류 0.
* X 버튼 — 저장된 씬 YAML 을 다시 읽어 <b>일곱이 전부</b> `aMin(1,1)` · `piv(1,1)` ·
  `pos(-27,-27)` · `32×32` 임을 확인한다.
* 성장 창 — `RelicBar` 1172×90 · `ChangeButton` y −42 · `RelicBar` 에 `Head` <b>없음</b> ·
  `Footer` <b>자식 0</b> · 씬 오브젝트 643 → **640**.
* 발굴 문구 — `ui_dig_title`/`_result`/`_boss` · `ui_dig_choice_*` · `relic_dialogue_*`(41) ·
  `dig_choice_text_*` 를 표에서 훑어 <b>영어 칸이 빈 줄이 하나도 없음</b>을 확인했다 —
  즉 한글이 나올 수 있는 길은 제목 하나뿐이었다.
* 로비 타이틀 — 구운 `LobbyTitle.png` 를 <b>눈으로 확인</b>(새 로고 · 알파 딴 자리 깨끗) ·
  저장된 `Lobby.unity` 에서 `Title` 762×444 · `Vignette` 1143×731 확인 ·
  `git status` 가 `LobbyTitle.png` <b>한 장만</b> 잡히는 것 확인.

### 아직 확인 못 한 것 (유저가 볼 것)

1. ★ **X 가 작아졌다** — 큰 창 셋(부대설정·전술지침·캐릭터성장)은 44→**32** 이다.
   원화가 32 라 이제야 픽셀이 안 번지지만, 누르기에 작아 보이면 일곱을 함께
   64(정확히 2배)로 올리면 된다 — 중간값은 다시 번진다.
2. ★ **유물 띠는 <b>예전 그대로</b>다** — 머리글만 사라졌다. 「Equipped Relics」가 없어져
   그 줄이 무엇인지 화면에 안 적혀 있다. 다시 필요하면 머리글을 <b>띠 위쪽 바깥</b>에
   (칸과 안 겹치게) 얹는 편이 낫다 — 띠 안에 도로 넣으면 같은 일이 반복된다.
3. ★ **`topReserved` 는 지금 0 이다** — 머리글이 없어 코드가 «비울 것 없음» 으로 잡는다.
   나중에 `RelicBar/Head` 를 되살리면 씬 높이만 키워 주면 칸이 알아서 내려간다.
4. ⚠ **환경설정 제목과 X 가 3px 겹친다**(UI-70-1 의 ⚠). 가운데 정렬이라 지금 문구로는
   안 닿지만, 더 긴 번역이 들어오면 닿을 수 있다.
5. ★ **새 타이틀은 로비를 실제로 띄워 볼 것** — 새 로고가 <b>가로로 더 넓다</b>(칸 757 → 762).
   스크립트는 «보이는 범위 위 26 ~ 아래 432px · 얼굴 470~575px 을 안 가린다» 로 계산했지만,
   배경 인물과의 겹침은 눈으로 봐야 한다. 가리면 `Title` 의 y(−7)만 올리면 된다.

### 씬 변경 여부

**있음.** `Proto_01` — X 버튼 일곱(자리·크기) · `HUD_Growth/RelicBar/Head` **삭제** ·
`HUD_Growth/Footer/CloseButton`(+`Label`) **삭제**.
**`Lobby`** — `Title` 757×444 → **762×444** · `Vignette` 1135×731 → **1143×731**.
⚠ `Fonts/NeoDunggeunmo SDF.asset` 이 같이 변경으로 잡힐 수 있다 — TMP 가 글자를 구워
  아틀라스에 쌓는 것이라 이 작업과 무관하다.
...
## UI-71. ★★★ 침식을 <b>«시간» 에서 «한 대» 로</b> — 시간 누적 삭제 · 후방 침식 철거 · 서서 맞는 만큼 쌓인다 (2026-09-02, 28차)

### 무엇을 / 왜

유저 지시 하나. 그대로 적는다.

> *"침식의 통제 수단이 사실 상 없어서 기존의 풀피 회복 시 침식 수치 감소 외에도 적에게
> 맞을때와 때릴때에 침식이 올라가고 <b>시간에 따라 올라가는 로직을 삭제</b>하여 전방의
> 아군은 확실하게 침식이 빠르게 올라가지만 위험할 때 후방으로 빼서 후퇴시킬 경우
> 관리가 가능하도록 변경"*

| # | 지시 | 결과 |
|---:|---|---|
| 1 | 적에게 <b>맞을 때</b> 침식이 올라간다 | ✅ `erosionPerHitTaken` 0.8 (UI-71-2) |
| 2 | 적을 <b>때릴 때</b> 침식이 올라간다 | ✅ `erosionPerHitDealt` 0.5 · ★★ 범위기는 <b>한 번만</b> 센다 (UI-71-2) |
| 3 | <b>시간에 따라 올라가는 로직을 삭제</b> | ✅ 「교전 중 초당 1.3」 · 「후방 침식」 두 갈래를 통째로 걷어냈다 (UI-71-1) |
| 4 | 전방은 확실하게 빨리, 후방으로 빼면 관리 가능 | ✅ 앞줄 2.9/초 ↔ 후방 0 (UI-71-3) |

**고친 C#** — `Combat/ErosionService.cs` · `Combat/CharacterErosion.cs` (둘뿐이다)
**씬 변경** — 있음 (`Proto_01` · `GameSystems/ErosionService` 직렬화 값 3칸 삭제 + 2칸 신설)
**에셋 변경** — 없음

⚠ **이 절 앞에 <b>2026-09-01 침식 작업이 로그에 없다</b>.** 그 두 커밋
(`b1a828c5` 공식 수정 · `dc3e685c` 침식 밸류 수정)이 지금 절이 <b>걷어내는 대상</b>이라,
읽는 사람이 «없던 것을 왜 지웠나» 가 되지 않게 UI-71-0 에 <b>사후 기록</b>으로 적어 둔다.
그 커밋의 침식 <b>이외</b> 부분(공식 개정 · 넥서스 체력바 · 히스톤 패시브 · 언어 추가 ·
웹빌드 압축)은 여전히 로그에 없다 — 이번 범위 밖이다.

---

### UI-71-0. (사후 기록) 하루 전에 무엇이 있었나 — 2026-09-01 의 두 걸음

이 절이 지우는 것이 무엇인지 알려면 그 전날이 필요하다. **직접 한 작업이 아니라 커밋과
코드에서 되짚어 적은 것**이다(형식 유지 · 요약 금지 규약대로 남기지만, 근거는 커밋이다).

#### ① `b1a828c5` 「공식 수정」 (13:41) — <b>후방 침식</b> 신설

유저 지시: *"후방에 있는 아군도 어느 정도는 침식이 되는 로직을 만들거나"*.

**무엇이 문제였나** — `CharacterErosion.IsInWaveCombat` 은 «내 타겟이 `Faction.Cancer` 이거나
/ 최근에 Cancer 에게 맞았거나» 였다. 그런데 <b>힐러는 타겟이 아군</b>이라 이 조건에
<b>영영 안 걸린다</b>. 원거리·보조도 사거리 밖에 서 있으면 0 이다. 그래서 침식이
«앞줄 몇 명만의 시스템» 이 됐고, 유저 체감이 *"침식도 사실상 안되는 느낌"* 이 됐다.

넣은 것:

```
   rearErosionPercent   30   웨이브 몬스터가 맵에 살아 있는 동안, 교전하지 않는 캐릭터가
                             전투 중 침식의 30% 를 쌓는다 (= 초당 0.39)
   recoverDelaySeconds   7 → 12   후방 침식이 회복 대기까지 되돌리므로 «웨이브가 끝난 뒤» 가
                                  유일한 회복 창이 됐다 — 그 창을 좁혔다
   erosionRecoverPerSecond 0.35 → 0.15   같은 이유로 빠지는 속도도 낮췄다
```

★ 조건을 `WaveManager` 가 아니라 <b>`UnitRegistry.FindFirst(Faction.Cancer, UnitKind.Monster)`</b>
  로 물었다 — 그 클래스에 static 진입점이 없고, «웨이브 단계» 와 «몬스터가 실제로 전장에
  있나» 는 미묘하게 다르다(진군 중·정리 중).

#### ② `dc3e685c` 「침식 밸류 수정」 (14:22) — <b>회복이 침식을 깎는다</b>

유저 지시: *"침식 밸류가 지나치게 높아짐 … 회복 시 침식이 어느 정도 낮아 진다거나 …
침식을 관리할 수 있는 수단이 필요"* → 이어서 *"아니 되돌리진 말고 관리 수단을 추가해
관리 수단 추가 되면 <b>지금 속도가 딱 괜찮을듯</b>"*.

★ 그래서 <b>쌓이는 속도는 한 칸도 안 건드렸다</b>. 대신 `erosionDropPerFullHeal = 12` 를
  넣고 `DamageableUnit.OnAnyHealed` 를 구독해, «최대 체력만큼 회복받으면 침식 12 감소» 를
  만들었다. <b>실제로 «찬» 체력만</b> 센다 — 만피에 힐을 부어도 안 줄어든다.

#### ★ 그런데 하루 만에 다시 «통제 수단이 없다» 가 온 이유

②의 회복 감소는 <b>살아 있고 이번에도 안 건드렸다</b>(유저 지시문의 «기존의 풀피 회복 시
침식 수치 감소 <b>외에도</b>» 가 그 뜻이다). 문제는 ①이었다.

★ 「교전 중 초당」과 「후방 초당」은 <b>둘 다 시간이 근거</b>다. 플레이어가 무엇을 하든
  똑같이 올라간다. 위험한 유닛을 뒤로 빼도 <b>후방 침식이 회복 대기(`_outOfCombatTime`)를
  매 프레임 0 으로 되돌려</b>, 웨이브가 끝날 때까지는 빠지지도 않았다.
  → 즉 <b>후퇴가 아무 일도 하지 않았다</b>. 회복 감소 하나로는 «관리» 가 되지 않는다.

---

### UI-71-1. 삭제 — 시간으로 쌓는 갈래를 <b>통째로</b> 걷어냈다

`CharacterErosion.Tick` 의 3분기(교전 중 / 후방 / 그 외)를 **1분기로** 줄였다.

```diff
-  if (IsInWaveCombat(service))                       // ① 교전 중 → 초당 1.3
-  { _outOfCombatTime = 0f; erosion += 1.3 * dt * gain; }
-  else if (waveMonstersAlive && RearErosionPercent > 0)  // ② 후방 → 초당 0.39
-  { _outOfCombatTime = 0f; erosion += 0.39 * dt * gain; }
-  else                                                // ③ 그 외 → 회복
-  { _outOfCombatTime += dt; if (…) erosion -= 0.15 * dt * recover; }
+  _outOfCombatTime += dt;                             // 언제나 흐른다
+  if (_outOfCombatTime >= RecoverDelaySeconds)
+      erosion -= ErosionRecoverPerSecond * dt * recover;
```
...
| 사라진 것 | 어디 | 왜 |
|---|---|---|
| `erosionPerSecondInCombat` (1.3) | ErosionService | 시간 누적 — 지시대로 삭제 |
| `rearErosionPercent` (30) · `RearErosionPerSecond` | ErosionService | 같음. 하루 만에 철거다 |
| `waveCombatMemorySeconds` (3) | ErosionService | «타겟을 놓쳐도 3초는 전투 중» 이라는 여유값 — 추론이 없어지니 쓸 곳이 없다 |
| `IsInWaveCombat()` | CharacterErosion | 같음 |
| `Update` 의 `UnitRegistry.FindFirst(Cancer, Monster)` | ErosionService | 후방 침식의 조건이었다 |
| `Tick` 의 `bool waveMonstersAlive` 인자 | CharacterErosion | 부르는 쪽이 하나뿐이라 그냥 지웠다 |

★ **한 톨도 안 남긴 이유** — 지시가 «삭제» 다. 초당 0.1 이라도 남겨두면 «뒤로 빼도 결국
  찬다» 가 되어 이번 변경의 뜻이 사라진다. `rearErosionPercent = 0` 으로 꺼두는 것과
  <b>지우는 것</b>을 굳이 갈랐다 — 칸이 남아 있으면 다음에 누가 «후방도 좀 쌓아야 하지
  않나» 로 되돌리고, 그러면 왜 지웠는지가 다시 잊힌다.

★ **`_outOfCombatTime` 의 뜻이 바뀌었다** — «웨이브 전투에서 벗어난 뒤» 에서
  <b>«마지막으로 한 대를 주고받은 뒤»</b> 가 됐다. 흐르는 곳은 `Tick` 하나,
  0 으로 되돌리는 곳은 `AddHitErosion` 하나다. <b>이 한 칸이 «후퇴시키면 관리된다» 를
  만든다</b> — 뒤로 빼면 대기시간이 차기 시작하고, `recoverDelaySeconds`(12) 뒤부터 빠진다.

---

### UI-71-2. 신설 — <b>«한 대» 를 주고받으면</b> 쌓인다

`ErosionService` 가 `DamageableUnit.OnAnyAttack` 을 구독한다. 진영·소환수 판정은 서비스가
하고, 값을 얹는 것은 `CharacterErosion.AddHitErosion(amount, dealt)` 이다 —
«규칙과 수치는 서비스 / 상태는 컴포넌트» 라는 이 파일 쌍의 기존 구조 그대로다.
...
```
   erosionPerHitDealt   0.5   웨이브 몬스터를 한 대 때릴 때
   erosionPerHitTaken   0.8   웨이브 몬스터에게 한 대 맞을 때
```

★ **맞는 쪽이 더 크다.** 맞는 것이 더 큰 압박이고, <b>앞줄과 뒷줄을 갈라놓는 값이 이것</b>이다.
  둘이 같으면 뒤에서 쏘는 원거리와 앞에서 버티는 탱커가 비슷하게 차 버린다.

#### ★★ 왜 `OnAnyDamaged` 가 아니라 `OnAnyAttack` 인가

처음엔 «실제로 깎인 피해» 를 싣는 `OnAnyDamaged` 를 쓰려 했다. 못 쓴다.

⚠ `OnAnyDamaged` 는 <b>때린 주체 없이</b> `ApplyDamage` 를 직접 부르는 경로에서도 뜬다
  (패시브 지속 피해 · 「가학증」의 자기 피해 · 라린길 「타오르는 숨결」의 체력 비례 추가 피해).
  그리고 그 이벤트가 싣는 공격자는 `DamageableUnit._lastAttacker` 인데, 이 칸은
  <b>지난번에 나를 때린 몬스터로 남아 있다</b>. 그대로 세면 «아무도 안 때렸는데 침식이
  오르는» <b>유령 누적</b>이 생긴다 — 시간 누적을 지운 자리에 시간 누적과 구분 안 되는
  것이 다시 들어오는 셈이다.

`OnAnyAttack` 은 `TakeDamageFrom` 한 번에 <b>정확히 한 번</b>, <b>공격자가 항상 유효한
상태로</b> 뜬다 — «한 대» 라는 근거와 정확히 같은 모양이다.

⚠ 대신 <b>빗나가도 뜬다</b>. 그대로 센다 — 이 값의 근거는 «피해» 가 아니라 «주고받는 행위»
  이고(칼을 휘두르는 것도 압박이다), 몬스터는 기본 명중 100% 라 «맞는 쪽» 은 어차피
  차이가 없다. 「맞을 때」쪽이 지시의 핵이므로 이쪽이 정확한 것이 중요하다.

#### ★★ 범위 공격은 <b>프레임당 한 번</b>만 센다

`UnitCombat` 의 광역 갈래는 <b>대상마다</b> `TakeDamageFrom(_self)` 을 부른다
(2413 · 2434 · 2458 · 2491 · 2496행). 그래서 이벤트도 대상 수만큼 뜬다.

⚠ 그대로 세면 여섯 마리를 쓸어담은 마법형이 <b>근접의 여섯 배</b>로 쌓여
  «범위기를 쓰면 미친다» 가 된다. 공격 유형이 침식 속도를 정하는 것은 이 시스템의 뜻이 아니다.

★ 그래서 「내가 때렸다」쪽만 <b>`Time.frameCount` 로 묶는다</b>. 한 번의 공격은
  <b>한 프레임 안에서</b> 전부 해결되고(동기 루프) 다음 공격은 `_nextAttackTime` 이 막으므로,
  <b>«프레임당 한 번» 이 곧 «공격당 한 번»</b> 이다. 공속을 아무리 올려도 어긋나지 않는다.
...
★ **「맞는 쪽」은 묶지 않는다.** 세 마리에게 둘러싸여 세 대 맞았으면 세 몫이 맞다.
  <b>앞줄이 빨리 차는 이유가 바로 이것</b>이라 여기서 묶으면 이번 변경이 무의미해진다.

#### 그대로 살아 있는 것

- **저항력 배율** — `ErosionGainMultiplier` 를 그 자리에서 곱한다. 저항 13 → 1.37배 ·
  저항 100 → 0.5배가 예전과 똑같이 작동한다.
- **유물 「서늘한 해열」**(`relic_erosion_slow`) — <b>쌓이는 쪽만</b> 늦춘다는 표의 정의문대로
  `AddHitErosion` 에만 곱하고 회복에는 안 곱한다.
- **소환수 제외** — 아루의 골렘. `Update` 루프와 <b>같은 판정</b>을 서비스 쪽에도 뒀다
  («누가 침식하는가» 는 한 규칙이다).
- **회복 시 침식 감소**(`erosionDropPerFullHeal = 12`) — UI-71-0 ②. 한 칸도 안 건드렸다.

★ **발동은 `AddHitErosion` 에서 하지 않는다.** 상한에 닿았는지 보는 곳은 `Tick` 한 곳뿐이다
  (늦어도 한 프레임 뒤에 발동한다). 발동 경로가 두 벌이 되면 «중첩 방지 · 지속 효과
  되돌리기» 를 양쪽에서 지켜야 하고, 이 파일은 그 실수를 이미 한 번 겪었다
  (`ClearActive` 를 한 곳으로 모은 이유).

---

### UI-71-3. 결과 — 어디에 세우는가가 곧 침식 관리다

몬스터 공속 ≈ 1.0 · 캐릭터 공속 ≈ 1.0 기준.

| | 새 값 | 옛 값 | 상한 100 까지 |
|---|---:|---:|---|
| 앞줄 탱커 (3마리에게 맞으며 때림) | **2.9/초** | 1.3/초 | 약 **35초** |
| 1마리와 맞붙은 근접 | 1.3/초 | 1.3/초 | 약 77초 |
| 뒤에서 쏘는 원거리 | 0.5/초 | 1.3 또는 0.39/초 | 약 200초 |
| 뒤에 세운 힐러 | **0** | 0.39/초 | ∞ (빠지기만) |

★ **한 마리와 맞붙은 근접이 정확히 1.3 이 되도록 값을 잡았다**(0.8 + 0.5) — 옛 «교전 중»
  값과 같다. 그래야 «앞줄이 빨라졌다» 가 <b>둘러싸인 만큼</b>이라는 뜻이 되고,
  1대1 로 싸우는 판의 체감이 어제와 같다. 기준점이 없으면 두 값이 그냥 임의의 숫자가 된다.

★ **후퇴의 값** — 뒤로 빼면 그 순간부터 아무것도 안 쌓이고, 12초 뒤부터 초당 0.15 로 빠진다.
  거기에 힐러의 회복이 «최대 체력의 10% 당 1.2» 로 더 깎는다. 후퇴 + 회복이 처음으로
  <b>둘 다 의미 있는</b> 상태가 됐다.

#### 회복 값(12 · 0.15)은 <b>일부러 안 건드렸다</b>

이 두 값은 UI-71-0 ①에서 «후방 침식이 생겨 쌓는 창이 넓어졌으니 빠지는 창도 좁힌다» 로
내려간 것이다. 그 후방 침식을 지금 철거했으니 되돌릴 명분이 있는데, 그러지 않았다.

★ 시간 누적이 사라진 것만으로 <b>빠지는 창이 이미 저절로 넓어졌다</b> — 예전엔 웨이브가
  도는 내내 대기시간이 0 으로 리셋됐지만, 지금은 <b>후퇴한 순간부터</b> 12초를 세기 시작한다.
  여기에 값까지 같이 올리면 <b>두 번 완화</b>하는 셈이고, 유저의 *"관리 수단 추가 되면
  지금 속도가 딱 괜찮을듯"* 이라는 판단선을 넘는다.
  → 부족하면 `recoverDelaySeconds` <b>한 칸만</b> 내리면 된다.

---

### UI-71-4. 씬 값을 코드와 <b>같이</b> 갈았다

이 파일 머리에 있는 규약: **직렬화된 값이 기본값을 이긴다.** 코드 기본값만 바꾸면
`Proto_01` 은 한 톨도 안 바뀐다. 그래서 씬의 `GameSystems/ErosionService` 블록을 같이 고쳤다.

```diff
-  erosionPerSecondInCombat: 1.3
-  waveCombatMemorySeconds: 3
-  rearErosionPercent: 30
+  erosionPerHitDealt: 0.5
+  erosionPerHitTaken: 0.8
   erosionDropPerFullHeal: 12
   recoverDelaySeconds: 12
   erosionRecoverPerSecond: 0.15
```

★ **MCP 가 아니라 YAML 직접 편집이다.** 유니티가 닫혀 있어 브리지가 죽어 있었고
  (`get_play_mode_status` 가 60초 큐에서 만료), 씬은 커밋 상태로 깨끗했다.
  바뀐 바이트는 정확히 −34 다 — 그 세 줄 ↔ 두 줄 차이뿐이다.
⚠ 유니티가 <b>열려 있는 동안</b>에는 이렇게 하면 안 된다. 메모리의 씬이 저장될 때
  덮어써서 편집이 조용히 사라진다.

⚠ **「능력치 및 공식 정리.xlsx」의 「계수」 시트에는 아직 옛 «초당» 줄이 남아 있다.**
  `erosionPerHitDealt` · `erosionPerHitTaken` 두 줄을 <b>새로 넣어야</b> 한다 —
  이 파일 머리 규약(기본값 ↔ 시트 일치)이 지금 깨진 상태다.

---

### 검증

★ **컴파일 확인됨 — 오류 0 · 경고 0.** 유니티가 닫혀 있어 MCP 로는 못 돌렸고,
  `D:\6000.3.19f1\Editor\Data\Managed` 의 유니티 DLL 을 참조하는 임시 csproj 를 만들어
  `Assets/**/*.cs`(Editor 폴더 제외)를 `dotnet build` 로 통째로 돌렸다.

⚠ **기존 `Assembly-CSharp.csproj` 로는 오류 13개가 뜬다 — 이 작업과 무관하다.**
  `<Compile>` 목록이 낡아서 `CharacterGender` · `RelicIconStrip` · `RelicCardStrip` ·
  `CharacterAltNames` · `NexusHealthBar` · `LanguageSetting` · `LanguagePickerPopup` 등
  <b>실제로 존재하는 파일들이 목록에 없다</b>(유니티가 열리면 재생성한다). 그래서
  파일 목록을 glob 으로 갈아서 검증했다 — 그때 0/0 이다.

### 남은 일 / 확인 요청

1. ★★ **런타임 검증이 남았다** — 플레이 모드로 ① 앞줄이 웨이브 2~3회에 발동하는지
   ② 후퇴시키면 게이지가 <b>멈추는지</b> ③ 12초 뒤 빠지기 시작하는지 ④ 범위기 캐릭터가
   근접보다 빨리 차지 않는지, 이 넷을 눈으로 봐야 한다. 이 프로젝트는 «컴파일은 되는데
   런타임에 안 돌던» 것을 여러 번 겪었다(플레이 모드에서만 잡히는 초기화 순서 함정).
2. ⚠ **힐러는 이제 침식이 정확히 0 이다.** 지시대로 시간 누적을 지운 <b>직접적 결과</b>이고
   (UI-71-0 ①이 후방 침식을 넣은 이유가 바로 이거였다), «세울 자리가 곧 관리» 라는 이번
   설계와는 일관된다. 다만 뒤에 세운 힐러는 정신 이상을 <b>영영 안 겪는다</b> —
   원치 않으면 «회복을 넣을 때도 소량» 같은 칸을 하나 더 열면 된다(유저 판단 대기).
3. ⚠ **도움말이 새 레버를 안 가르친다.** `Resources/Data/Help/HelpTable.asset` 의
   *"보라색 게이지가 침식입니다. 싸우는 동안 차오르고, 싸움이 끝나면 저절로 줄어듭니다."*
   는 틀리지는 않았지만 «후퇴가 관리 수단» 이라는 이번 핵심을 안 알려준다.
   9개 언어 문안이라 지시 없이 손대지 않았다.
4. ⚠ **「계수」 시트 두 줄** (UI-71-4 의 ⚠).
5. ⚠ **로그가 여전히 밀려 있다** — `2cc36b7b`(원거리 몬스터가 성역을 못 때리던 버그) ·
   `956ff739`·`06b1acf0`(오프닝/엔딩·타이틀) · `29383961`(아이오 페이지) ·
   `5647d7f7`(히스톤 복수자·사건 창 강제·삼킨 티끌 하향) · `6f0ccca6`(웹빌드 압축) ·
   `b1a828c5`/`dc3e685c` 의 <b>침식 이외</b> 부분 · `b6958a03`·`3ba30dfe`(언어 추가)
   가 이 로그에 없다. UI-71-0 은 그중 침식 부분만 메운 것이다.

### 씬 변경 여부

**있음.** `Proto_01` — `GameSystems/ErosionService` 의 직렬화 값
`erosionPerSecondInCombat`·`waveCombatMemorySeconds`·`rearErosionPercent` <b>삭제</b> ·
`erosionPerHitDealt`(0.5)·`erosionPerHitTaken`(0.8) <b>신설</b>. 오브젝트·계층은 안 건드렸다.

### 씬반영요청 목록

- 없음 (YAML 을 직접 고쳤다 — UI-71-4 의 ★ 참조)
