...
| **U-S2** | **HUD 는 씬에 오브젝트를 만들지 않고 코드로 런타임 생성한다.** `[RuntimeInitializeOnLoadMethod]` 로 부팅해 기존 `UI_Root` 캔버스 아래에 계층을 만든다(§10). 씬 파일이 **한 바이트도 안 바뀐다** — 이 방식을 기본으로 유지할 것. |
| **U-S3** | 씬 저장이 꼭 필요한 경우(템플릿 인스펙터 값 변경, 새 루트 오브젝트)에만 저장하고, **커밋을 따로 분리**해 메시지에 `(씬)` 을 붙인다. 무엇을 왜 바꿨는지 `작업로그_UI.md` 에 남길 것. |
...
| **U-S8** | 구식 UI 오브젝트 `UI_Root/Energy`, `UI_Root/Upgrade` 는 **씬에서 지우지 말고 런타임에 비활성화**한다(§10). 씬 재작성을 피하기 위해서다. |
...
## §10. HUD 구현 방침 (이번 스코프에서 확정)

| 방침 | 내용 |
|---|---|
| **H-1 코드 생성** | HUD 계층 전체를 **런타임에 코드로 만든다.** 씬에 UI 오브젝트를 추가하지 않는다 — 38MB 씬 재작성을 피하고, 레이아웃이 코드라 리뷰·머지가 가능해진다. |
| **H-2 부팅** | `HudBootstrap` 의 `[RuntimeInitializeOnLoadMethod(AfterSceneLoad)]` 가 진입점. 기존 `UI_Root` 캔버스를 찾아 그 아래에 붙이고, 없으면 캔버스를 새로 만든다. |
| **H-3 구식 UI** | `UI_Root/Energy`, `UI_Root/Upgrade` 는 부팅 시 **런타임 비활성화**한다. 씬에서 삭제하지 않는다(U-S8). |
| **H-4 폰트 — 네오둥근모 고정** | `Assets/_Project/Resources/Fonts/NeoDunggeunmo SDF.asset` 을 `Resources.Load<TMP_FontAsset>("Fonts/NeoDunggeunmo SDF")` 로 읽는다. **런타임 생성 UI는 인스펙터 참조를 가질 수 없어서** 폰트를 `Resources/` 로 옮겼다(GUID 유지 — 씬의 기존 TMP 참조는 그대로 살아있다). 다른 폰트로 바꾸거나 새로 굽지 말 것. |
| **H-5 스타일 한 곳** | 색·여백·글자 크기는 `HudTheme` 한 곳에 모은다. 패널마다 흩어놓지 말 것. |
| **H-6 해상도** | 기존 `UI_Root` 의 CanvasScaler 설정(1920×1080, Match 0.5)을 그대로 쓴다. |
