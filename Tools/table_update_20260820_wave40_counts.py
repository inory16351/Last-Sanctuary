...
``wave_top_boss`` 위에서부터:
    ...
    25→120001 · 30→120002 · 35→120003 · 40→120004     ← 한 바퀴 돌아 위에서부터

⚠⚠ **베일(120005)은 아직 이 목록에 없다** — 인게임 스킨이 아직 없어서다.
  자세한 이유와 다시 넣는 법은 :data:`BOSS_ROTATION` 주석에 있다.
...
#:
#: ⚠⚠ **베일(120005)은 아직 빠져 있다** (2026-08-20). 표·스트링·일러스트는 다 들어왔지만
#:   <b>인게임 스킨(프레임)이 없다</b> — 원화 시트의 프레임 수가 확정되지 않아서다
#:   (`Tools/bale_skin_build.py` 맨 위 ⚠⚠: 헤더는 「16프레임」인데 실제로 그려진 것은
#:    14장이고, 라벨 번호도 건너뛰거나 중복된다. 줄마다 값이 달라 자동으로는 못 정한다).
#:   스킨이 없는 보스를 넣으면 `ResolveBossSlot` 이 경고를 내고 **기본 보스로 대신
#:   내보낸다** — 표에 적힌 보스와 화면에 나오는 보스가 달라진다. 그래서 뺐다.
#:   스킨이 준비되면 이 목록 끝에 ``120005`` 를 붙이고 이 스크립트를 다시 돌리면 된다.
