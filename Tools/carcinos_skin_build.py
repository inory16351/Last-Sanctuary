...
SRC = find_art("Carcinos_asset_02.png")
...
SRC_FX = find_art("Carcinos_asset_01.png")
...
def main():
    ...
    for motion in WANTED_ROWS:
        ...
        # ★ 가로는 <b>발 중심</b>을, 세로는 <b>바닥</b>을 캔버스에 맞춘다(위 ★★).
        #   피벗이 (0.5, 0) = 발밑이라 바닥을 맞춰야 모션이 바뀔 때 위아래로 안 튄다.
        rgbas = [to_rgba(arr[b[2]:b[3] + 1, b[0]:b[1] + 1], bg) for b in boxes]
        oxs, w = foot_layout(rgbas)
        for i, (bx0, bx1, by0, by1) in enumerate(boxes):
            rgba = rgbas[i]
