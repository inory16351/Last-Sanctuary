# -*- coding: utf-8 -*-
"""바리올라(1103) 서식지 줄을 `habitat_design` 에 추가한다 (2026-08-20).

유저 지시: *"바리올라 서식지 청크 이미지 넣었으니까 분석해서 타일맵 만들고 (스프라이트
폴더에 있음) 테이블에도 값 추가해줘"*
...
지금 `habitat_design` 에는 1101(카르시노스) · 1102(아니사킬) 두 줄뿐이고 **1103 이 없다.**

값을 어떻게 정했나
==================
| 컬럼 | 값 | 근거 |
|---|---|---|
| `habitat_tile_asset` | `VariolaHabitat` | `gen_variola_habitat_tiles.py` 가 만든 폴더 이름. 코드가 여기에 `Edge`·`Props` 를 붙여 찾는다(`NeutralMonsterDefinitionSO.HabitatPath`) |
...
동급» 으로 맞춰 놨고, 바리올라도 `neutrality_mon` 에서 완전히 같은 등급이다
...
⚠ 「생성될 때마다 랜덤 서식지 배정 · 저장 시 위치 보존」은 **이미 되어 있다.**
  ...
  바리올라는 그 경로를 **그대로 탄다** — 빠져 있던 것은 타일과 이 표 한 줄뿐이다.
...
def backup():
    ...
    dst = os.path.join(BACKUP_ROOT, stamp + "_바리올라서식지")
...
def main():
    ...
    for asset, *_ in ROWS.values():
        ...
        if not os.path.isdir(folder):
            raise SystemExit("⚠ 타일 폴더가 없습니다 (gen_variola_habitat_tiles.py 를 "
                             "먼저 돌리세요): " + folder)
