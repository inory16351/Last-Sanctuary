...
★★ 발밑 「드롭 섀도」를 지운다 (2026-08-19)
------------------------------------------
유저 리포트: *"카시노마 밑에 검은색 그림자있는거 없애줘"*.

원화에는 캐릭터 **발밑에 회색 드롭 섀도**가 그려져 있다. 밝기만 보는 배경 판정으로는
이게 안 걸러진다 — 그림자 픽셀은 (174,174,177)·(152,149,153) 처럼 **광도 150~190**
이라 :data:`BG_LUM_LO`(150) 아래로 내려가는 순간 **완전 불투명**이 되기 때문이다.
그래서 유닛이 회색 얼룩을 달고 다녔다.

⚠ **밝고 무채색이면 지운다** 로는 안 된다 — 이 캐릭터의 뼈·발톱·이빨도 밝고 채도가
  낮아서 같이 지워진다. 그래서 **테두리에서 흘려 채운다**(flood fill): 시트 <b>바깥과
  이어진</b> 무채색 밝은 덩어리만 배경으로 확정한다. 몸 안의 흰 발톱은 검은 외곽선에
  둘러싸여 있어 흐름이 닿지 못한다. 넥서스 원화가 같은 이유로 같은 방법을 썼다(102-7절).

⚠ **돌진 잔상(연기)은 지우면 안 된다.** 다행히 그 연기는 붉은 기가 남아 있어
  (채도 > :data:`SHADOW_SAT_MAX`) 조건에 안 걸린다 — 실측으로 잔상 4장에서
  사라진 불투명 픽셀은 52개(전부 발밑 그림자)였다.

⚠ scipy 는 쓰지 않는다. 이 저장소의 다른 스크립트(`nexus_skin_build.py`)가 쓰고 있지만
  **없는 PC 가 있다.** numpy 슬라이싱 전파로 같은 결과를 0.7초에 낸다(실측 623회 반복 ·
  BFS 결과와 픽셀 단위로 일치).
...
#: 발밑 드롭 섀도 판정 (맨 위 ★★ 참조). 이 두 조건을 <b>둘 다</b> 만족하면서
#: <b>시트 테두리와 이어진</b> 픽셀만 배경으로 확정한다.
#:
#: ⚠ 채도 상한을 올리면(35+) **돌진 잔상의 연기**까지 먹는다. 광도 하한을 내리면
#:   (110 이하) 몸의 어두운 회색 부위로 흐름이 새어 들어간다. 실측으로 20/130 이
#:   그림자만 정확히 잡는다(잔상 4장에서 사라진 불투명 픽셀 52개 = 전부 그림자).
SHADOW_SAT_MAX = 20
SHADOW_LUM_MIN = 130
...
def outside_mask(lum, sat):
    """
    <b>시트 테두리와 이어진</b> 무채색·밝은 덩어리 = 배경 + 발밑 드롭 섀도 (맨 위 ★★).

    "밝고 무채색이면 배경" 으로 끝내면 몸 안의 흰 발톱·이빨까지 지워진다. 그것들은
    검은 외곽선에 둘러싸여 <b>바깥과 이어져 있지 않다</b> — 그래서 이어짐을 따진다.

    scipy 없이 numpy 슬라이싱으로 전파한다. 배경이 넓어 반복이 수백 번 돌지만 한 번에
    150만 픽셀을 처리하므로 1초가 안 걸린다(실측 623회 · 0.7초).
    """
...
def to_rgba(rgb):
    ...
    # ★ 발밑 드롭 섀도 — 위 밝기 기준으로는 <b>불투명</b>으로 남는다(광도 150~190).
    #   테두리에서 흘려 채워 바깥과 이어진 덩어리만 지운다.
    alpha[outside_mask(lum, sat)] = 0
