...
------------------------
  · 이동 → **좌/우 전용 원화 두 줄**.
    ⚠⚠ **시트 제목("이동 (Move / Walk) - 좌 / 우")을 믿으면 안 된다** — 제목대로
    첫 줄을 왼쪽으로 읽었는데 **그림은 반대**였고, 그래서 2026-08-20 까지 엘린만
    **바라보는 방향과 걷는 방향이 반대**였다(유저 리포트). 실측 근거는 BODY_ROWS 주석.
    이 작가의 시트는 헤더 숫자도 틀린 적이 있다(120절 — 베일·바리올라).
...
BODY_ROWS = [
    ...
    # ★★ 2026-08-20 — <b>이동 두 줄의 방향이 서로 바뀌어 있었다.</b>
    #    유저 리포트: *"엘린캐릭터가 지금 바라보는 방향과 걷는 방향이 반대로 되있어"*.
    #
    #    원인은 <b>시트 제목을 믿은 것</b>이다 — "이동 (Move / Walk) - 좌 / 우" 라고
    #    적혀 있어 첫 줄(y 204~288)을 `"left"`, 둘째 줄(y 313~395)을 `"right"` 로 뒀다.
    #    아래 write 루프가 `side == "right"` 만 `Right` 로 쓰므로 첫 줄이 그대로
    #    `Char_Walk_Left_*` 로 나갔다. 그런데 <b>그림은 그 반대</b>다.
    #
    #    ⚠ 눈으로는 판단이 갈려서 <b>지표 두 개를 따로 재서</b> 확인했다. 기준자는
    #    <b>미러로 만든 줄</b>이다 — 근거리·대기는 원본이 오른쪽을 보는 것이 확실하다
    #    (사슬이 오른쪽으로 뻗는다). 그 줄과 <b>같은 값</b>이 나오는 쪽이 오른쪽이다:
    #
    #      ① 두건의 <b>금색 십자</b> 가로 위치 (흰 두건 폭에 대한 비율)
    #           기준자: 근거리 0.60 · 대기 0.52   (= 오른쪽)
    #           첫 줄 0.61 ← 같다                 둘째 줄 0.42 ← 다르다
    #      ② <b>은발 덩어리</b> 중심 − 실루엣 중심 (걸을 때 머리는 <b>뒤로</b> 흐른다)
    #           기준자: 근거리 오른쪽 −5.6px · 근거리 왼쪽 +5.6px
    #           첫 줄 −8.4px ← 오른쪽              둘째 줄 +8.0px ← 왼쪽
    #
    #    두 지표가 같은 답을 낸다: <b>첫 줄 = 오른쪽 · 둘째 줄 = 왼쪽</b>.
    #    그래서 <b>side 값만 서로 바꾼다</b> — y 좌표는 시트 실측값이라 그대로 둔다.
    #
    #    ⚠ 다른 캐릭터는 이 함정에 안 걸렸다 — 이동 줄이 <b>좌/우 전용 두 줄인 캐릭터가
    #      엘린뿐</b>이고(나머지는 한 줄을 미러한다) 미러는 방향이 어긋날 수가 없다.
    ("Walk",         "right",  204, 288, 294, 303, None),
    ("Walk",         "left",   313, 395, 401, 410, None),
