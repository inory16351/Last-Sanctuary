...
def build_relic_panel():
    ...
    # ★★ <b>설명·서사 칸을 키웠다</b> (2026-08-24 · 유저 지시:
    #   *"유물 ui안에 텍스트 배치할때 텍스트가 짤리지 않도록"*).
    #   상세 틀은 422x388 인데 예전에는 설명 78px · 서사 76px 만 쓰고 <b>서사 아래 54px 를
    #   비워 두고</b> 있었다(서사 바닥 −250 · 출처 머리 −304). 그 빈 자리를 두 칸에 나눠 준다.
    #   설명 78 → <b>94</b>(16pt 로 넉 줄) · 서사 76 → <b>110</b>(14pt 로 여섯 줄).
    #   가장 긴 설명이 38자 · 가장 긴 서사가 45자이므로 두 줄이면 끝난다 — 넉넉하다.
    # ⚠ 그래도 «절대 안 넘친다» 는 보장은 <b>코드가</b> 한다(`HudTheme.FitText`) —
    #   TMP 의 줄바꿈·자동 크기 칸은 MCP 로 넘길 수 없다(위 `text()` 의 ⚠⚠).
    #   여기서 하는 일은 «자동 축소가 웬만해선 필요 없게» 칸을 넉넉히 잡는 것이다.
    label(d + "/Effect", "", 16, TEXT, "TopLeft", (0, 1), (1, 1), (14, -186), (-14, -92))
    label(d + "/Flavor", "", 14, DIM, "TopLeft", (0, 1), (1, 1), (14, -300), (-14, -190))
    # ⚠ <b>「장착」 버튼(y 8~44)과 겹치면 안 된다</b> (2026-08-25 수정).
    #   예전 값은 Wearer 가 y 34~58 이라 <b>10px 겹쳤고</b>, Wearer 는 왼쪽 정렬인데
    #   버튼도 왼쪽(x 14~150)에 있어서 «누가 착용 중인지» 가 버튼에 통째로 가렸다.
    #   두 줄을 버튼 위로 올린다 — Wearer 46~70, Source 74~100.
    label(d + "/Source", "", 13, DIM, "Left", (0, 0), (1, 0), (14, 74), (-14, 100))
    label(d + "/Wearer", "", 13, DIM, "Left", (0, 0), (1, 0), (14, 46), (-14, 70))
