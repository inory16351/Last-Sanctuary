...
FACES = [
    ...
    # ★★ 2026-08-26 — <b>(545, 383, 654) 에서 다시 찍었다</b> (유저 지시:
    #   *"아르세니아 일러스트 팔 없는거 보이게 해줘"* → *"이렇게 말고 <b>팔이 오른쪽이</b> 나와야지"*).
    #   ⚠ <b>이 원화에는 팔이 둘 다 잘려 있었다.</b> 처음엔 왼쪽(포션을 든 팔)을 담게 잡았다가
    #     유저가 <b>오른팔</b>이라고 정정했다 — 잘린 쪽이 어디인지 먼저 물었어야 했다.
    #   옛 값의 크롭은 (left 303 · right 787 · top 56 · bottom 710) 이었다. 인물의 오른팔은
    #   원본 <b>x≈700~840 · y≈530~760</b> 에 내려와 있어 <b>오른쪽과 아래 양쪽에서</b> 잘렸다 —
    #   자주색 장갑을 낀 소매 끝이 통째로 프레임 밖이라 «팔이 없는» 그림이 됐다.
    #   ★ 그래서 오른쪽을 <b>911</b> 까지, 아래를 <b>870</b> 까지 넓혔다(손 끝 760 뒤로 110px 여유).
    #     비율이 묶여 있어 가로를 넓히면 세로도 따라 커진다(484x654 → 621x840).
    #   ⚠ <b>왼쪽 포션 팔은 그대로 잘린다</b> — 둘 다 담으려면 가로 695(x 168~863)가 필요하고
    #     그러면 크롭 높이가 940 이 되어 <b>얼굴이 초상화 칸에서 너무 작아진다</b>. 유저가 고른
    #     것은 오른팔이다.
    #   ★ 규칙 ①③은 지켰다 — 머리 위 여백 45px(크롭 높이의 5.4%) · 눈높이가 위쪽 29%.
    ('illust_Arsenia.png', 'illust_Arsenia', 600,  450, 840),  # 1024x1536 — 후드 + 날개 + 오른팔
    ('illust_Vulcan.png',  'illust_Vulcan',  545,  455, 540),  # 1024x1536 — 가시관 + 수염·날개
    # 2026-08-21 신규 캐릭터 3인(엘리시아 9012 · 세라피엘 9013 · 시안 9014).
    # 같은 세 기준(머리 위 여백 5~8% · 가로 중심 · 눈높이 위쪽 1/3)으로 잡았다.
    # ⚠ <b>시안만 원본이 가로로 눕는다</b>(1402x1122) — 다른 열셋은 세로 전신샷이다.
    #   그래서 크롭 높이를 620 으로 낮췄다(700 이면 세로가 원본의 62% 라 어깨 아래가
    #   통째로 들어와 얼굴이 작아진다).
    ('illust_Elysia.png',    'illust_Elysia',    614, 633, 700),  # 1122x1402 — 가시관 + 큰 방패
    ('illust_Seraphiel.png', 'illust_Seraphiel', 437, 508, 700),  # 1055x1491 — 후광 + 후드 + 쌍권총
    ('illust_Cyan.png',      'illust_Cyan',      691, 393, 620),  # 1402x1122 — 후광 + 모자 + 큰 낫
    # ★★ 2026-08-20 — <b>아루의 골렘</b>(「강림」 80024). 유저 지시:
    #   *"아루 골렘 일러스트 연동(Aru_dawn_illust)"*.
    #   ⚠ 이것은 <b>캐릭터 표에 행이 없는 유닛</b>이다 — 골렘은 플레이어가 뽑는 인물이 아니라
    #     스킬이 부르는 소환수이고, 정의 에셋도 런타임에 만든다(<c>AruGolem.BuildDefinition</c>).
    #     그래서 `gen_character_assets.py` 가 이 크롭을 만들어 주지 않는다 — 여기에 손으로 적는다.
    #   ★ 크롭 기준은 다른 열둘과 같다(머리 위 여백 6% · 가로 중심 · 얼굴이 위쪽 1/3).
    #     골렘은 <b>후광이 머리보다 위</b>에 있어(원본 y≈90) 그 위로 여백을 잡았다.
    ('illust_AruDawn.png', 'illust_AruDawn', 561,  320, 520),  # 1122x1402 — 후광 + 어깨·가슴
]
